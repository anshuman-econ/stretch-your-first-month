...
function ExplainerScreen({ onContinue, onClose }: { onContinue: () => void; onClose: () => void }) {
  ...
  const unlockCards = [
    ["Care", "Specialist routing matched to your pathway — peri, endo, derm, metabolic, or longevity."],
    ["Coach", "A coach turns the plan into weekly actions, so it does not stay theoretical."],
    ["Labs", "When labs matter, Stretch connects them back to your plan instead of leaving you with a confusing PDF."],
    ["Pods", "Guided small-group sessions that help you act, not just learn."],
    ["Pass", "A monthly experience — movement, recovery, LED, breathwork, workshop, or partner demo."],
    ["Kit", "A guided shelf matched to your month: supplements, pantry tools, visible vitality, or recovery support."],
  ];
  const groups = [
    ...
    { title: "Experiences", status: "swappable", items: ["Pilates / Barre seats", "pelvic-floor support", "red-light sessions", "LED booth", "glow-up routines", "partner demos"], what: "One real-world pass or partner moment that makes the month feel usable.", when: "Appears when there is a good match for your goal, location, and partner availability." },
    { title: "Kits", status: "pack-only", items: ["magnesium support", "collagen minis", "smart snack packs", "broths", "HA-ceramide support", "SPF / skin support"], what: "At-home support matched to sleep, comfort, glow, pantry, or recovery.", when: "Appears after the plan knows what would help this month without overloading your shelf." },
  ...
  return <section className="absolute inset-0 z-50 flex flex-col bg-shell p-4 pb-6"><div className="mb-4 flex items-center justify-between gap-3"><div><p className="text-sm font-bold text-accent">Member explainer</p><h1 className="font-display text-3xl leading-tight">What’s Stretch?</h1></div><button onClick={onClose} className="rounded-full bg-secondary px-4 py-2 text-sm font-bold text-accent shadow-card focus-visible:outline-none focus-visible:ring-2 focus-visible:ring-ring">Close</button></div><div className="flex-1 space-y-5 overflow-y-auto pr-1"><div className="rounded-[2rem] bg-hero p-5 shadow-float"><h2 className="font-display text-4xl leading-tight">Stop piecing care together alone.</h2><p className="mt-3 text-sm leading-6 text-muted-foreground">Stretch builds your first month around what you want to feel better in 30 days — with care, coaching, pods, a guided kit, an experience pass, and the next unlocks already mapped.</p><p className="mt-3 rounded-2xl bg-card/85 p-3 text-xs font-bold leading-5 text-accent shadow-card">No fixed bundle. No endless marketplace. A guided plan you can actually use.</p><div className="mt-5 grid gap-3"><Button variant="hero" size="lg" onClick={onContinue}>Start quiz</Button><Button variant="soft" size="lg" onClick={() => setShowExampleMonthPreview(true)}>See an example month</Button></div></div><ExplainerSection title="What you unlock"><div className="grid grid-cols-2 gap-3">{unlockCards.map(([title, copy]) => <button key={title} onClick={() => setDetail({ title, copy, status: "Can include" })} className="rounded-3xl bg-card p-4 text-left shadow-card transition-smooth hover:-translate-y-0.5 focus-visible:outline-none focus-visible:ring-2 focus-visible:ring-ring"><p className="text-xs font-bold uppercase tracking-wide text-accent">{title}</p><p className="mt-2 line-clamp-2 text-sm leading-5 text-muted-foreground">{copy}</p></button>)}</div><p className="rounded-2xl bg-secondary p-3 text-center text-sm font-bold text-accent">Stretch connects the pieces. You do not have to.</p></ExplainerSection><ExplainerSection title="Why it is different"><div className="grid gap-3"><CompareCard title="Static programs" copy="One fixed bundle, even when your symptoms change." muted /><CompareCard title="Open marketplaces" copy="Lots of choices, but you still have to figure out what belongs together." muted /><CompareCard title="Stretch" copy="A guided month built for you. Keep it, swap one thing, or ask your coach to rebalance it." highlight /></div><p className="rounded-2xl bg-card p-4 text-sm font-bold leading-6 text-accent shadow-card">Stretch is not “here are 100 options.” Stretch is “here is the month we built for you — and here is what you can safely adjust.”</p></ExplainerSection><div className="overflow-hidden rounded-[2rem] bg-card p-5 shadow-card"><p className="mb-4 font-display text-2xl leading-tight">What Stretch can include or unlock</p><div className="group flex gap-2 overflow-hidden"><div className="flex shrink-0 animate-marquee gap-2 group-hover:[animation-play-state:paused]">{chips.map((chip) => <button key={chip} onClick={() => setDetail({ title: chip, status: "Pathway-dependent", copy: `${chip} can appear when it matches your pathway, month, eligibility, and partner availability.` })} className="shrink-0 rounded-full bg-secondary px-4 py-2 text-xs font-bold text-accent shadow-card focus-visible:outline-none focus-visible:ring-2 focus-visible:ring-ring">{chip}</button>)}</div><div aria-hidden="true" className="flex shrink-0 animate-marquee gap-2 group-hover:[animation-play-state:paused]">{chips.map((chip) => <span key={`repeat-${chip}`} className="shrink-0 rounded-full bg-secondary px-4 py-2 text-xs font-bold text-accent shadow-card">{chip}</span>)}</div></div><div className="mt-4 grid gap-3">{groups.map((group) => <button key={group.title} onClick={() => openGroup(group)} className="rounded-3xl bg-secondary p-4 text-left shadow-card transition-smooth hover:-translate-y-0.5 focus-visible:outline-none focus-visible:ring-2 focus-visible:ring-ring"><div className="flex items-center justify-between gap-3"><p className="font-display text-xl leading-tight">{group.title}</p><span className="rounded-full bg-card px-3 py-1 text-[10px] font-bold text-accent">{group.status}</span></div><div className="mt-3 flex flex-wrap gap-2">{group.items.slice(0, 3).map((item) => <span key={item} className="rounded-full bg-card px-3 py-2 text-xs font-bold text-muted-foreground shadow-card">{item}</span>)}</div></button>)}</div><p className="mt-4 text-xs font-semibold leading-5 text-muted-foreground">What you see depends on your pathway, month, eligibility, and partner availability. Stretch shows what is relevant now — and what can open later.</p></div><div className="rounded-[2rem] bg-card p-5 shadow-card"><p className="text-sm font-bold text-accent">Example month</p><h2 className="mt-1 font-display text-3xl leading-tight">Example: Peri Sleep + Energy Month</h2><p className="mt-2 text-sm leading-6 text-muted-foreground">Because you selected poor sleep + fatigue, Stretch might build:</p><div className="mt-4 grid gap-2">{[["Care", "peri-aware review"], ["Coach", "sleep routine + symptom rhythm"], ["Pods", "Peri Reset + Sleep Reset"], ["Kit", "magnesium + sleep support + visible vitality item"], ["Pass", "breathwork or cooling reset"], ["Unlock next", "Smart Ring preview + PeriShield Rx later"]].map(([title, copy]) => <div key={title} className="flex items-start justify-between gap-3 rounded-2xl bg-secondary px-4 py-3"><p className="text-xs font-bold uppercase tracking-wide text-accent">{title}</p><p className="max-w-[68%] text-right text-sm font-semibold leading-5 text-muted-foreground">{copy}</p></div>)}</div><Button variant="hero" size="lg" className="mt-4 w-full" onClick={onContinue}>Show me my version</Button></div><button onClick={openMbc} className="w-full rounded-[2rem] bg-card p-5 text-left shadow-card transition-smooth hover:-translate-y-0.5 focus-visible:outline-none focus-visible:ring-2 focus-visible:ring-ring"><p className="text-sm font-bold text-accent">Milestone Bonus Credits</p><h2 className="mt-1 font-display text-3xl leading-tight">Progress should unlock more care.</h2><p className="mt-3 text-sm leading-6 text-muted-foreground">Build kit + join pod + book pass → earn progress credits → unlock a pack discount, kit upgrade, or device buy-down later.</p><div className="mt-4 grid grid-cols-2 gap-3"><div className="rounded-2xl bg-secondary p-3"><p className="text-xs font-bold uppercase tracking-wide text-accent">Earn with</p><p className="mt-2 text-xs leading-5 text-muted-foreground">kit, pod, lab, pass, coaching, 7-day streak</p></div><div className="rounded-2xl bg-secondary p-3"><p className="text-xs font-bold uppercase tracking-wide text-accent">Use for</p><p className="mt-2 text-xs leading-5 text-muted-foreground">packs, devices, kit upgrades, add-ons, partner experiences</p></div></div><p className="mt-4 rounded-2xl bg-sand p-3 text-xs font-bold leading-5 text-sand-foreground">MBC is not insurance cashback. It is a Stretch reward that helps you unlock the next layer of support.</p></button><div className="rounded-[2rem] bg-hero p-5 shadow-float"><h2 className="font-display text-3xl leading-tight">Ready to see your first month?</h2><div className="mt-5 grid gap-2"><Button variant="hero" size="lg" onClick={onContinue}>Start quiz</Button><Button variant="soft" size="lg" onClick={() => setShowExampleMonthPreview(true)}>See example month</Button><Button variant="soft" size="lg" onClick={onClose}>Close</Button></div></div></div>{detail && <ExplainerDetailDrawer detail={detail} onClose={() => setDetail(null)} />}{showExampleMonthPreview && <ExampleMonthPreview onClose={() => setShowExampleMonthPreview(false)} onStart={() => { setShowExampleMonthPreview(false); onContinue(); }} />}</section>;
}
