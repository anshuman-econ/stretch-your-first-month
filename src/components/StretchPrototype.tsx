...
const pathways: Record<PathwayKey, Pathway> = {
  peri: {
    ...
    vitality: "HA-ceramide, collagen, and skin-hair Included preview.",
  ...
  endo: {
    ...
    foundation: ["Endo-aware gyne routing", "Choose one pain-aware experience", "Two coach touches", "Pain psychology micro-Included", "Endo Toolkit Pod"],
    ...
    tabs: {
      ...
      Coach: ["Touch 1: flare map + pacing plan", "Touch 2: function / GI / routine adjustment", "Pain psychology micro-Included + Mood Check", "Pod choice: GI/Function Support, Pain Pacing, or Fatigue Support"],
    ...
    vitality: "Confidence and comfort Included preview.",
  ...
  metabo: {
    ...
    sustain: ["Unlock actives or Camera-Ready Sprint", "Open LED Mask preview", "Add coverage Included when relevant"],
  ...
  longevity: {
    ...
    sustain: ["Unlock Brain Sprint", "Open Smart Ring preview", "Add lab or sleep Included if useful"],
...
const Locked previewCards = ["Fertility / IVF", "Surgery Track", "full GLP route", "advanced riders"];
...
const quiz = [
  ...
  { question: "What kind of Included do you want first?", options: ["symptom relief", "labs and answers", "coaching and accountability", "visible results", "movement / recovery", "prescription / rider Included later", "a 90-day plan"] },
...
const rebalanceOptions = ["more sleep Included", "more energy Included", "more glow / visible vitality", "more mood Included", "more movement Included", "less intensity this month"];
...
const pathwayDefaults: Record<PathwayKey, Record<string, string>> = {
  ...
  endo: {
    ...
    "Mental Support": "pain psychology micro-Included + Mood Check",
    "Clinical / LED / Review": "recovery review, LED relax, or documentation review",
    Labs: "CBC, ferritin, Vitamin D, B12 conditional, TVUS Included",
...
const mentalCatalog = ["Mood Check", "Micro-CBT", "pain psychology micro-Included", "CBT-i / anxiety Included", "body image / stress Included", "binge trigger / stress-eating Included", "performance / identity stress Included", "full mental-health session, quarterly or pack-based"];
...
const diagnosticsCatalog: Record<PathwayKey, string[]> = {
  ...
  endo: ["Included: CBC, ferritin, Vitamin D, B12 conditional, TVUS Included", "GI selective: stool calprotectin, celiac tTG-IgA, H. pylori selective", "MRI: Locked preview / pre-auth / rider-Includeded if indicated"],
...
const packsCatalog: Record<PathwayKey, string[]> = {
  ...
  metabo: ["Derma Actives Pack", "Camera-Ready Sprint", "Protein & Pantry Pack", "Procedure Token Booster", "Hair Actives Pack", "Hair Shedding Guard", "GLP packs Locked preview"],
...
const catalogForBlock = (block: MonthBlock, pathway: Pathway): { rule: string; options: CatalogOption[]; agenda?: string } => {
  ...
  if (block.name === "Clinical / LED / Review") return { rule: "Choose 1 route per month if Included. High-cost procedures are tokenized or Pack-only.", options: reviewCatalog.map((name, i) => ({ name, state: i > 10 ? "Locked preview / Pack-only" : i > 5 ? "tokenized" : "Pick 1" })) };
...
const passportStamps: PassportStamp[] = [
  ...
  { title: "Kit built", means: "Your Included items are selected for this month.", complete: "Pick your kit defaults and confirm shipping preferences.", unlocks: "Badge only unless configured.", mbc: "complete kit build = badge only unless configured" },
            ...
            <section className="space-y-7 px-5 pb-8 pt-6">
              <div className="relative overflow-hidden rounded-[2rem] bg-hero p-6 shadow-float">
                ...
                <div className="relative flex min-h-[520px] flex-col justify-between">
                  <div className="flex items-center justify-between"><p className="font-display text-3xl text-Recommended">Stretch</p><span className="rounded-full bg-card/80 px-3 py-1 text-xs font-semibold text-foreground shadow-card backdrop-blur">30 days</span></div>
              ...
              <div className="grid grid-cols-2 gap-3">{Locked previewCards.map((item) => <div key={item} className="rounded-3xl border border-border bg-secondary p-4 text-muted-foreground shadow-card"><Lock className="mb-3 size-4" /><p className="font-medium">{item}</p><p className="mt-1 text-xs">Unlocks later</p></div>)}</div>
            </section>
          )}
...
function QuizScreen({ quizIndex, chooseAnswer }: { quizIndex: number; chooseAnswer: (answer: string) => void }) {
  return <section className="space-y-6 px-5 py-7"><div className="space-y-3"><p className="text-sm font-semibold text-accent">Question {quizIndex + 1} of 6</p><div className="h-2 overflow-hidden rounded-full bg-secondary"><div className="h-full rounded-full bg-Recommended transition-smooth" style={{ width: `${((quizIndex + 1) / 6) * 100}%` }} /></div></div><SectionTitle title={quiz[quizIndex].question} /><div className="grid gap-3">{quiz[quizIndex].options.map((option) => <SoftCard key={option} onClick={() => chooseAnswer(option)} className="p-4 capitalize"><span className="flex items-center justify-between gap-3"><span>{option}</span><ArrowRight className="size-4 text-accent" /></span></SoftCard>)}</div></section>;
}
...
function UnlocksScreen({ pathway, answers, onBuild, onKeep, onSwap }: { pathway: Pathway; answers: string[]; onBuild: () => void; onKeep: () => void; onSwap: () => void }) {
  ...
  const cards = [
    ...
    { title: "Your monthly kit", lead: "Guided kit builder", copy: "You’ll build a guided kit with Included items, food-structure tools, and one visible-vitality item." },
...
function BuilderScreen({ pathway, onStart, onCoach }: { pathway: Pathway; onStart: () => void; onCoach: () => void }) {
  ...
  const updated = rebalance ? [
    ...
    "Changed kit item: one home Included item is adjusted",
...
function NavItem({ icon, label, active, onClick }: { icon: React.ReactNode; label: string; active: boolean; onClick: () => void }) {
  return <button onClick={onClick} className={cn("flex flex-col items-center gap-1 rounded-2xl px-1 py-2 text-[11px] font-medium transition-smooth focus-visible:outline-none focus-visible:ring-2 focus-visible:ring-ring", active ? "bg-secondary text-Recommended" : "text-muted-foreground hover:text-foreground")}>{icon}<span>{label}</span></button>;
}
...
function BehindScenesPanel({ onClose }: { onClose: () => void }) {
  ...
  const memberCards = [
    ...
    ["Progress Engine", "AI watches your streaks, logs, labs, and attendance to unlock Milestone Bonus Credits and future Included."],
  ];
  const operatorCards = [
