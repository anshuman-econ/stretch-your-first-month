...
function UnlocksScreen({ pathway, onBuild, onKeep, onSwap }: { pathway: Pathway; answers: string[]; onBuild: () => void; onKeep: () => void; onSwap: () => void }) {
  ...
  const activation = activationForPathway(key);
  const podNames = activation.pods.split(" + ");
  const openSimpleDetail = (title: string, eyebrow: string, copy: string, chips?: string[], action = "Keep recommended") => setDetail({ title, eyebrow, copy, chips, action });
  return <section className="relative space-y-5 px-5 py-7"><div className="space-y-2"><p className="text-sm font-bold text-accent">Activation Review</p><h1 className="font-display text-4xl leading-tight">Your Stretch month is ready.</h1><p className="text-base leading-7 text-muted-foreground">Review what unlocked, choose what needs your input, and start your first week.</p></div><div className="sticky top-0 z-10 -mx-5 bg-shell/95 px-5 py-2 backdrop-blur-xl"><div className="flex gap-2 overflow-x-auto pb-1">{sectionTabs.map((tab) => <button key={tab} onClick={() => setActive(tab)} className={cn("shrink-0 rounded-full px-4 py-2 text-xs font-bold transition-smooth focus-visible:outline-none focus-visible:ring-2 focus-visible:ring-ring", active === tab ? "bg-primary text-primary-foreground shadow-card" : "bg-card text-accent shadow-card")}>{tab}</button>)}</div></div>{active === "Stack" && <ActivationPanel eyebrow="Section 1" title="Your Month Stack" copy="Give a visual overview of the full month. Do not repeat only the pathway."><div className="grid gap-3">{[["Pathway", pathway.title, "Your matched route for this first month."], ["Why selected", pathway.reason, "This explains why Stretch chose the month stack."], ["Care route", d.Specialist, "Specialist routing anchors the care side of the plan."], ["Coach + pods", `${d.Coaching} • ${d.Pods}`, "Coaching turns the pod work into weekly actions."], ["Kit", d.Kit, "At-home support between care, coaching, and pods."], ["Pass", d["Experience Pass"], "One bookable monthly experience."], ["Progress", "Milestone Bonus Credits for completing plan actions", "Credits are earned by finishing the actions that move the month forward."], ["Future unlocks", d["Future Unlocks"], "Previews of what may open later without forcing it today."]].map(([label, value, context]) => <ActivationInfoCard key={label} label={label} copy={value} onOpen={() => openSimpleDetail(label, "Your Month Stack", context, value.split(/, | • /))} />)}</div><Button variant="hero" size="xl" className="mt-4 w-full" onClick={() => setActive("Care")}>Review care</Button></ActivationPanel>}{active === "Care" && <ActivationPanel eyebrow="Section 2" title="Care + Labs" copy="Specialist, functional, clinical, LED, review, and diagnostic options are shown with the exact route Stretch unlocked for this pathway."><div className="grid gap-3"><ActivationInfoCard label="Specialist route" copy={activation.specialist} onOpen={() => openSimpleDetail("Specialist route", "Care + Labs", "The specialist route is the expert lane Stretch uses to keep the plan clinically grounded and matched to your pathway.", activation.specialist.split(" / "))} /><ActivationInfoCard label="Functional support / A2 options" copy={activation.functional} onOpen={() => openSimpleDetail("Functional support / A2 options", "Care + Labs", "Functional support is the hands-on or guided support layer you can choose from when the plan needs movement, nutrition, recovery, LED, acupuncture, or coaching support.", activation.functional.split(", "))} /><ActivationInfoCard label="Clinical / LED / review route" copy={activation.clinical} onOpen={() => openSimpleDetail("Clinical / LED / review route", "Care + Labs", "This is the review or in-clinic route that helps Stretch decide whether the month should stay simple, add recovery support, or escalate later.", activation.clinical.split(", "))} /><ActivationInfoCard label="Diagnostics / labs" copy={activation.labs} onOpen={() => openSimpleDetail("Diagnostics / labs", "Care + Labs", "Labs appear when relevant to the pathway and are used to clarify fatigue, symptoms, metabolism, inflammation, hormones, or recovery signals.", activation.labs.split(/, | \/ /))} /></div><ActivationButtons primary="Choose functional support" secondary="Keep recommended" onPrimary={() => openSimpleDetail("Choose functional support", "Care + Labs", "Choose from the functional support options Stretch matched to this pathway. These options are meant to help the month feel actionable, not overwhelming.", activation.functional.split(", "))} onContinue={() => setActive("Coach")} onSecondary={onKeep} /></ActivationPanel>}{active === "Coach" && <ActivationPanel eyebrow="Section 3" title="Coach + Pods" copy="Selected pods, what each pod covers, coaching touch 1, coaching touch 2, and mental / behavioral support if relevant."><div className="grid gap-3"><ActivationInfoCard label="Selected pods" copy={activation.pods} onOpen={() => setDetail({ title: "Selected pods", eyebrow: "Coach + Pods", copy: "These are the pod seats selected for the pathway. Each pod gives the month a guided agenda, outputs, and a reason to keep moving.", rows: podNames.map((pod) => ({ label: pod, copy: podAgendas[pod] || activation.podCovers })), action: "View pod agenda" })} /><ActivationInfoCard label="What each pod covers" copy={activation.podCovers} onOpen={() => setDetail({ title: "What each pod covers", eyebrow: "Coach + Pods", copy: "The pod agenda explains the topic, the working rhythm, and the output you leave with.", rows: podNames.map((pod) => ({ label: pod, copy: podAgendas[pod] || activation.podCovers })), action: "View pod agenda" })} /><ActivationInfoCard label="Coaching touch 1" copy={activation.coach1} onOpen={() => openSimpleDetail("Coaching touch 1", "Coach + Pods", "The first coaching touch sets up the routine, map, or cadence so your first week starts clearly.", [activation.coach1])} /><ActivationInfoCard label="Coaching touch 2" copy={activation.coach2} onOpen={() => openSimpleDetail("Coaching touch 2", "Coach + Pods", "The second coaching touch adjusts the plan after you start using it, so the month stays realistic.", [activation.coach2])} /><ActivationInfoCard label="Mental / behavioral support" copy={activation.mental} onOpen={() => openSimpleDetail("Mental / behavioral support", "Coach + Pods", "This support is included when mood, stress, pain, sleep, adherence, body image, or brain fog can make the month harder to complete.", activation.mental.split(" + "))} /></div><ActivationButtons primary="View pod agenda" secondary="Swap one pod" onPrimary={() => setDetail({ title: "Pod agenda", eyebrow: "Coach + Pods", copy: "Each pod includes a focused agenda and practical output for the month.", rows: podNames.map((pod) => ({ label: pod, copy: podAgendas[pod] || activation.podCovers })), action: "Continue" })} onContinue={() => setActive("Kit")} onSecondary={onSwap} /></ActivationPanel>}{active === "Kit" && <ActivationPanel eyebrow="Section 4" title="Kit + Perks" copy="Your kit is your at-home support between care, coaching, and pods."><div className="grid gap-3"><ActivationInfoCard label="Recommended kit" copy={activation.kit} onOpen={() => openSimpleDetail("Recommended kit", "Kit + Perks", "This is the pathway-specific kit Stretch selected to support the work you are doing at home between care, coaching, and pods.", kitCatalog[key])} /><ActivationInfoCard label="What can be swapped" copy="One kit item, one comfort or vitality lane, or one pantry / support item depending on the pathway." onOpen={() => openSimpleDetail("What can be swapped", "Kit + Perks", "Kit swaps keep the same pathway and purpose while letting you adjust one item or support lane.", kitCatalog[key])} /><ActivationInfoCard label="Sticky perk option" copy={stickyPerks.slice(0, 4).join(", ")} onOpen={() => openSimpleDetail("Sticky perk option", "Kit + Perks", "Sticky perks are small engagement perks that make the month feel more useful and rewarding.", stickyPerks)} /><ActivationInfoCard label="Pack-only upgrades if relevant" copy={packsCatalog[key].slice(0, 3).join(", ")} onOpen={() => openSimpleDetail("Pack-only upgrades if relevant", "Kit + Perks", "Pack-only upgrades are not default month items. They can appear when the pathway, progress, or later need makes them relevant.", packsCatalog[key])} /></div><ActivationButtons primary="Build kit" secondary="Swap one kit item" onPrimary={() => openSimpleDetail("Build kit", "Kit + Perks", "Build the kit from the recommended pathway-specific support and swap one eligible item if needed.", kitCatalog[key])} onContinue={() => setActive("Pass + Unlocks")} onSecondary={onSwap} /></ActivationPanel>}{active === "Pass + Unlocks" && <ActivationPanel eyebrow="Section 5" title="Pass + Progress + Future Unlocks" copy="Experience pass, MBC context, earn actions, uses, and pathway-specific future unlocks."><div className="grid gap-3"><ActivationInfoCard label="Experience pass" copy={activation.passes} onOpen={() => openSimpleDetail("Experience pass", "Pass + Progress + Future Unlocks", "You can pick one monthly pass from the pathway-specific experience options.", activation.passes.split(", "), "Choose pass")} /><ActivationInfoCard label="MBC context" copy="Milestone Bonus Credits are Stretch-funded credits you earn for completing plan actions." onOpen={() => setDetail({ title: "Milestone Bonus Credits", eyebrow: "Pass + Progress + Future Unlocks", copy: "Milestone Bonus Credits are Stretch-funded credits you earn for completing plan actions.", rows: [{ label: "Earn with", copy: "kit built, pod joined, pass booked, labs completed, coaching done, 7-day streak" }, { label: "Use for", copy: "selected packs, kit upgrades, device buy-downs, future pathway add-ons, special partner experiences" }], action: "Learn about MBC" })} /><ActivationInfoCard label="Earn with" copy="kit built, pod joined, pass booked, labs completed, coaching done, 7-day streak" onOpen={() => openSimpleDetail("Earn with", "Pass + Progress + Future Unlocks", "These are the completion actions that can earn Milestone Bonus Credits.", ["kit built", "pod joined", "pass booked", "labs completed", "coaching done", "7-day streak"])} /><ActivationInfoCard label="Use for" copy="selected packs, kit upgrades, device buy-downs, future pathway add-ons, special partner experiences" onOpen={() => openSimpleDetail("Use for", "Pass + Progress + Future Unlocks", "Credits can be used toward selected next-step support when the plan is ready for it.", ["selected packs", "kit upgrades", "device buy-downs", "future pathway add-ons", "special partner experiences"])} /><ActivationInfoCard label="Future unlocks" copy={activation.future} onOpen={() => openSimpleDetail("Future unlocks", "Pass + Progress + Future Unlocks", "These pathway-specific previews can open later through progress, eligibility, review, or the next best step.", activation.future.split(", "), "See future unlocks")} /></div><div className="mt-4 grid gap-3"><Button variant="soft" size="lg" onClick={() => openSimpleDetail("Choose pass", "Pass + Progress + Future Unlocks", "Pick one monthly pass from the options available for this pathway.", activation.passes.split(", "))}>Choose pass</Button><Button variant="soft" size="lg" onClick={() => setDetail({ title: "Milestone Bonus Credits", eyebrow: "Pass + Progress + Future Unlocks", copy: "Milestone Bonus Credits are Stretch-funded credits you earn for completing plan actions.", rows: [{ label: "Earn with", copy: "kit built, pod joined, pass booked, labs completed, coaching done, 7-day streak" }, { label: "Use for", copy: "selected packs, kit upgrades, device buy-downs, future pathway add-ons, special partner experiences" }], action: "Continue" })}>Learn about MBC</Button><Button variant="soft" size="lg" onClick={() => openSimpleDetail("Future unlocks", "Pass + Progress + Future Unlocks", "These pathway-specific previews can open later through progress, eligibility, review, or the next best step.", activation.future.split(", "))}>See future unlocks</Button><Button variant="hero" size="xl" onClick={onBuild}>Go to full plan</Button></div></ActivationPanel>}{detail && <ActivationDetailDrawer detail={detail} onClose={() => setDetail(null)} />}</section>;
}
