...
function ExplainerScreen({ onContinue, onExample }: { onContinue: () => void; onExample: () => void }) {
  const [detail, setDetail] = useState<ExplainerDetail | null>(null);
  const stack = ["Care", "Coach", "Labs", "Pods", "Pass", "Kit"];
  const chips = ["gynae check-ins", "peri-aware reviews", "endo support", "Pilates / Barre seats", "pelvic-floor support", "red-light sessions", "LED face masks", "glow-up routines", "skin / hair consults", "magnesium support", "collagen minis", "smart snack packs", "Oura-style smart rings", "CGM windows", "home lab panels", "community pods", "mood check-ins", "nutrition support", "future riders", "milestone credits"];
  const chipStatus = (chip: string) => chip.includes("ring") || chip.includes("CGM") || chip.includes("riders") ? "future unlock" : chip.includes("LED face") || chip.includes("home lab") ? "pack-only" : chip.includes("gynae") || chip.includes("endo") || chip.includes("nutrition") ? "matched when useful" : "can be included or swappable";
  const inside = [
    { title: "Care", copy: "Specialist check-ins and clinician-guided next steps, matched to your pathway.", examples: ["peri-aware women’s health review", "endo-aware gyne review", "derm review", "internal medicine review"] },
    { title: "Coach", copy: "Your coach turns the plan into weekly actions — sleep routines, flare pacing, food cadence, glow routines, focus blocks, or movement goals." },
    { title: "Labs", copy: "When labs are useful, Stretch ties them to a plan instead of leaving you with a report to decode alone.", examples: ["CBC", "ferritin", "Vitamin D", "TSH / FT4", "HbA1c", "lipids", "ApoB"] },
    { title: "Pods", copy: "Small guided group sessions that make the plan easier to follow.", examples: ["Peri Reset Pod", "Endo Toolkit Pod", "Skin Sunday Pod", "Brain & Focus Pod"] },
    { title: "Pass", copy: "A monthly experience pass that can route you into a class, clinic moment, recovery session, demo, or workshop.", examples: ["Pilates", "breathwork", "red-light", "LED booth", "skin routine demo", "pelvic relaxation"] },
    { title: "Kit", copy: "A guided kit or shelf matched to your month.", examples: ["magnesium", "omega-3", "sleep tea", "broths", "SPF", "HA-ceramide", "collagen", "protein", "smart snack packs"] },
    { title: "Unlocks", copy: "As your pattern becomes clearer, Stretch can unlock packs, devices, riders, and new pathways.", examples: ["Smart Ring", "LED Mask", "CGM", "PeriShield Rx", "DermaShield+", "Longevity Lab Rider"] },
  ];
  return <section className="space-y-7 px-5 py-7"><div className="rounded-[2rem] bg-hero p-6 shadow-float animate-enter"><div className="flex items-start justify-between gap-4"><div><h1 className="font-display text-4xl leading-tight">What’s Stretch?</h1><p className="mt-3 text-base leading-7 text-muted-foreground">Stretch builds your first month of care around what you want to feel better in 30 days.</p></div><Sparkles className="size-7 shrink-0 text-accent" /></div><div className="mt-6 rounded-[2rem] bg-card/85 p-4 shadow-card backdrop-blur"><div className="relative h-40">{stack.map((item, index) => <button key={item} onClick={() => setDetail({ title: item, status: "month stack", copy: `${item} is one guided part of your first month.` })} className="absolute left-1/2 flex w-[82%] -translate-x-1/2 items-center justify-between rounded-2xl border border-border bg-card px-4 py-3 text-sm font-bold shadow-card transition-smooth hover:-translate-y-0.5 focus-visible:outline-none focus-visible:ring-2 focus-visible:ring-ring" style={{ top: `${index * 20}px`, zIndex: stack.length - index }}><span>{item}</span>{iconForBlock(item)}</button>)}</div></div><p className="mt-5 text-sm leading-6 text-muted-foreground">Most wellness programs give you a fixed bundle. Stretch builds a guided monthly stack — care, coaching, labs, pods, experiences, kits, and future unlocks — around what you actually care about.</p><div className="mt-5 grid gap-3"><Button variant="hero" size="xl" onClick={onContinue}>Take the quiz <ArrowRight className="size-4" /></Button><Button variant="soft" size="xl" onClick={onExample}>Show me an example month</Button></div></div><div className="overflow-hidden rounded-[2rem] bg-card p-5 shadow-card"><p className="mb-4 text-sm font-bold text-accent">A wellness-first subscription that can include or unlock…</p><div className="group flex gap-2 overflow-hidden"><div className="flex shrink-0 animate-marquee gap-2 group-hover:[animation-play-state:paused]">{[...chips, ...chips].map((chip, index) => <button key={`${chip}-${index}`} onClick={() => setDetail({ title: chip, status: chipStatus(chip), copy: `${chip} ${chipStatus(chip)} depending on your pathway, progress, availability, and what is safe for this month.` })} className="shrink-0 rounded-full bg-secondary px-4 py-2 text-xs font-bold text-accent shadow-card focus-visible:outline-none focus-visible:ring-2 focus-visible:ring-ring">{chip}</button>)}</div></div></div><ExplainerSection title="Why Stretch is different"><div className="grid gap-3"><CompareCard title="Static programs" copy="Most monthly programs give you the same fixed bundle, even if your symptoms change." muted /><CompareCard title="Open marketplaces" copy="Marketplaces give you lots of options, but you still have to figure out what to choose." muted /><CompareCard title="Stretch" copy="Stretch builds a guided month for you. You can keep the plan, swap one thing, or ask your coach to rebalance it." highlight /></div></ExplainerSection><ExplainerSection title="How your month is built"><div className="grid gap-3">{[["1", "Tell us what you want to feel better in 30 days", "sleep, energy, glow, pain, focus, mood, cravings"], ["2", "Stretch recommends a pathway", "Peri Sleep + Energy, Endo Flare + Function, MetaboGlow Camera-Ready, Longevity Brain + Focus"], ["3", "Your month unlocks", "a care route, coach plan, pod seats, kit, experience pass, and progress passport"], ["4", "You choose the parts that matter", "You are not choosing from hundreds of random services. Stretch gives you a recommended stack and lets you safely adjust selected parts."], ["5", "Your progress opens more", "Complete your plan, earn Milestone Bonus Credits, and unlock future packs, devices, riders, and adjacent pathways."]].map(([number, title, copy]) => <VisualStep key={number} number={number} title={title} copy={copy}><div className="h-2 rounded-full bg-secondary"><div className="h-full w-2/3 rounded-full bg-primary" /></div></VisualStep>)}</div></ExplainerSection><ExplainerSection title="What you get inside Stretch"><div className="flex gap-3 overflow-x-auto pb-2">{inside.map((item) => <button key={item.title} onClick={() => setDetail({ ...item, status: "inside Stretch" })} className="min-w-[210px] rounded-[2rem] bg-card p-5 text-left shadow-card transition-smooth hover:-translate-y-0.5 focus-visible:outline-none focus-visible:ring-2 focus-visible:ring-ring"><div className="mb-4 grid size-12 place-items-center rounded-2xl bg-secondary text-accent">{iconForBlock(item.title)}</div><h2 className="font-display text-2xl">{item.title}</h2><p className="mt-2 line-clamp-3 text-sm leading-6 text-muted-foreground">{item.copy}</p></button>)}</div></ExplainerSection><ExplainerSection title="Pick what matters, without starting from scratch"><div className="rounded-[2rem] bg-card p-5 shadow-card"><div className="grid gap-2">{["Keep recommended", "Swap one thing", "Ask coach to rebalance"].map((item, index) => <button key={item} className={cn("rounded-full px-4 py-3 text-sm font-bold shadow-card", index === 0 ? "bg-primary text-primary-foreground" : "bg-secondary text-accent")}>{item}</button>)}</div><p className="mt-4 text-sm leading-6 text-muted-foreground">Stretch builds your month first. Then you can adjust selected parts — like your pod, experience pass, kit item, or functional session — without having to design your own care plan from zero.</p><div className="mt-4 grid gap-2">{[["I want more sleep support", "Stretch may rebalance toward Sleep Reset Pod, magnesium / sleep kit, breathwork pass, and sleep coaching."], ["I want more glow", "Stretch may rebalance toward Skin Sunday Pod, HA-ceramide, LED / aftercare pass, and visible vitality support."], ["I want less intensity this month", "Stretch may keep your care route but simplify your pass and kit."]].map(([title, copy]) => <button key={title} onClick={() => setDetail({ title, status: "example rebalance", copy })} className="rounded-2xl bg-secondary p-4 text-left text-sm font-semibold text-foreground">“{title}”</button>)}</div></div></ExplainerSection><ExplainerSection title="Progress should unlock more care"><div className="rounded-[2rem] bg-card p-5 shadow-card"><p className="text-sm leading-6 text-muted-foreground">Milestone Bonus Credits are Stretch-funded credits you earn by completing the actions that make your plan work.</p><div className="mt-4 flex gap-2 overflow-x-auto pb-1">{["build your kit", "join your pod", "complete your lab", "book your experience pass", "finish your first coaching touch", "keep a 7-day tracking streak", "improve sleep regularity", "complete Month 1"].map((item) => <span key={item} className="shrink-0 rounded-full bg-secondary px-3 py-2 text-xs font-bold text-accent">{item}</span>)}</div><div className="mt-4 grid grid-cols-2 gap-2 text-xs font-semibold text-muted-foreground">{["selected packs", "device buy-downs", "kit upgrades", "future pathway add-ons", "special partner experiences"].map((item) => <span key={item} className="rounded-2xl bg-muted px-3 py-3">{item}</span>)}</div><p className="mt-4 rounded-2xl bg-sand p-3 text-xs font-bold leading-5 text-sand-foreground">MBC is not insurance cashback. It is a Stretch reward that helps you unlock the next layer of support.</p></div></ExplainerSection><ExplainerSection title="Your first Stretch journey"><div className="grid gap-3">{["Day 1 · Take quiz", "Day 1 · Stretch builds your month", "Day 2 · Pick kit + pods + pass", "Week 1 · Start coaching + track symptoms", "Week 2 · Join pod + adjust plan", "Week 3 · Use pass or kit", "Week 4 · Review progress and see next unlock"].map((item) => <div key={item} className="flex items-center gap-3 rounded-2xl bg-card p-4 shadow-card"><span className="size-3 rounded-full bg-primary" /><p className="text-sm font-bold">{item}</p></div>)}<p className="rounded-2xl bg-secondary p-4 text-sm leading-6 text-muted-foreground">You are not meant to figure this out alone. Stretch gives you a path, then helps you keep moving.</p></div></ExplainerSection><ExplainerSection title="Where Stretch can take you next"><div className="grid grid-cols-2 gap-3">{["Peri Sleep + Energy", "Endo Flare + Function", "MetaboGlow Camera-Ready", "Longevity Brain + Focus", "Fertility / IVF", "Full GLP route", "Surgery Track", "Advanced riders"].map((item, index) => <div key={item} className={cn("rounded-3xl p-4 shadow-card", index > 3 ? "bg-muted text-muted-foreground" : "bg-card")}><Lock className={cn("mb-2 size-4", index > 3 ? "text-muted-foreground" : "text-accent")} /><p className="font-display text-lg leading-tight">{item}</p><p className="mt-1 text-xs font-bold text-accent">{index > 3 ? "locked preview" : "pathway"}</p></div>)}</div><p className="mt-4 text-sm leading-6 text-muted-foreground">Start with one thing you want to feel better in 30 days. Stretch can open the next layer when you are ready.</p></ExplainerSection><div className="rounded-[2rem] bg-primary p-6 text-primary-foreground shadow-float"><h2 className="font-display text-3xl leading-tight">Ready to build your first month?</h2><div className="mt-5 grid gap-3"><Button variant="soft" size="xl" onClick={onContinue}>Start quiz</Button><Button variant="soft" size="xl" onClick={onExample}>Show me an example plan</Button></div></div>{detail && <ExplainerDetailDrawer detail={detail} onClose={() => setDetail(null)} />}</section>;
}
...
function ExplainerDetailDrawer({ detail, onClose }: { detail: ExplainerDetail; onClose: () => void }) {
  return <div className="absolute inset-0 z-50 flex items-end bg-primary/25 p-3 backdrop-blur-sm" onClick={onClose}><div className="max-h-[78vh] w-full overflow-y-auto rounded-[2rem] bg-card p-6 shadow-float animate-slide-up" onClick={(event) => event.stopPropagation()}><div className="mb-4 flex items-start justify-between gap-3"><div><p className="text-sm font-bold text-accent">{detail.status || "Stretch detail"}</p><h2 className="font-display text-3xl leading-tight">{detail.title}</h2></div><button onClick={onClose} className="rounded-full bg-secondary px-3 py-2 text-sm font-semibold text-accent">Close</button></div><p className="text-sm leading-6 text-muted-foreground">{detail.copy}</p>{detail.examples && <div className="mt-4 flex flex-wrap gap-2">{detail.examples.map((item) => <span key={item} className="rounded-full bg-secondary px-3 py-2 text-xs font-bold text-accent">{item}</span>)}</div>}<Button variant="hero" size="lg" className="mt-5 w-full" onClick={onClose}>Got it</Button></div></div>;
}

function QuizScreen({ quizIndex, chooseAnswer, onExplain }: { quizIndex: number; chooseAnswer: (answer: string) => void; onExplain: () => void }) {
  return <section className="space-y-6 px-5 py-7"><div className="flex justify-end"><button onClick={onExplain} className="inline-flex items-center gap-2 rounded-full bg-card px-3 py-2 text-xs font-bold text-accent shadow-card transition-smooth hover:-translate-y-0.5 focus-visible:outline-none focus-visible:ring-2 focus-visible:ring-ring"><Info className="size-4" />What’s Stretch?</button></div><div className="space-y-3"><p className="text-sm font-semibold text-accent">Question {quizIndex + 1} of 6</p><div className="h-2 overflow-hidden rounded-full bg-secondary"><div className="h-full rounded-full bg-primary transition-smooth" style={{ width: `${((quizIndex + 1) / 6) * 100}%` }} /></div></div><div><SectionTitle title={quiz[quizIndex].question} /><button onClick={onExplain} className="mt-3 text-sm font-bold text-accent underline-offset-4 hover:underline focus-visible:outline-none focus-visible:ring-2 focus-visible:ring-ring">New here? See how Stretch works.</button></div><div className="grid gap-3">{quiz[quizIndex].options.map((option) => <SoftCard key={option} onClick={() => chooseAnswer(option)} className="p-4 capitalize"><span className="flex items-center justify-between gap-3"><span>{option}</span><ArrowRight className="size-4 text-accent" /></span></SoftCard>)}</div></section>;
}
...
function UnlocksScreen({ pathway, answers, onBuild, onKeep, onSwap }: { pathway: Pathway; answers: string[]; onBuild: () => void; onKeep: () => void; onSwap: () => void }) {
  ...
  const cardFor = (title: string) => summaries.find((card) => card.title === title)?.recommendation || "Unlocked for this month";
  const cards = [
    { icon: <Sparkles className="size-5" />, title: "Pathway unlocked", lead: pathway.title, copy: `Because you selected ${answerText}.`, status: "Unlocked" },
    { icon: <ClipboardList className="size-5" />, title: "Care route unlocked", lead: cardFor("Care"), copy: "Start with a safe, symptom-first review.", status: "Ready" },
    { icon: <Star className="size-5" />, title: "Pods unlocked", lead: cardFor("Pods"), copy: "Two guided group sessions this month.", status: "Seats ready" },
    { icon: <Package className="size-5" />, title: "Kit unlocked", lead: cardFor("Kit"), copy: "Build a kit matched to your symptoms.", status: "Build next" },
    { icon: <Ticket className="size-5" />, title: "Experience pass unlocked", lead: cardFor("Pass"), copy: "Pick one experience this month.", status: "Choose" },
    { icon: <Wallet className="size-5" />, title: "Progress passport unlocked", lead: "Earn Milestone Bonus Credits", copy: "Complete steps to unlock future credits.", status: "0 of 5" },
    { icon: <Lock className="size-5" />, title: "Future unlocks opened", lead: "Packs, devices, and coverage add-ons", copy: "Available as your pattern becomes clearer.", status: "Preview" },
  ];
  const card = cards[revealIndex];
  const continueReveal = () => revealIndex === cards.length - 1 ? setComplete(true) : setRevealIndex((current) => current + 1);
  return <section className="flex min-h-full flex-col px-5 py-7"><div className="space-y-2"><h1 className="font-display text-4xl leading-tight">Your Stretch month is ready</h1><p className="text-base leading-7 text-muted-foreground">Here’s what unlocked for you.</p></div><div className="mt-6 flex flex-1 flex-col justify-center"><div key={card.title} className="min-h-[390px] rounded-[2rem] bg-card p-6 shadow-float animate-enter"><div className="flex h-full min-h-[342px] flex-col justify-between"><div><div className="mb-6 flex items-center justify-between gap-3"><div className="rounded-full bg-secondary p-4 text-accent shadow-card">{card.icon}</div><span className="rounded-full bg-secondary px-3 py-1 text-xs font-bold text-accent">{card.status}</span></div><p className="text-sm font-bold text-accent">{card.title}</p><h2 className="mt-3 font-display text-4xl leading-tight">{card.lead}</h2><p className="mt-4 text-base leading-7 text-muted-foreground">{card.copy}</p></div><Button variant="hero" size="xl" className="mt-8 w-full" onClick={continueReveal}>Continue <ArrowRight className="size-4" /></Button></div></div><div className="mt-5 flex justify-center gap-2">{cards.map((item, index) => <span key={item.title} className={cn("h-2 rounded-full transition-all", index === revealIndex ? "w-8 bg-primary" : index < revealIndex || complete ? "w-2 bg-accent" : "w-2 bg-secondary")} />)}</div></div>{complete && <div className="mt-6 grid gap-3 animate-slide-up"><Button variant="hero" size="xl" className="w-full" onClick={onBuild}>Build my month</Button><Button variant="soft" size="xl" className="w-full" onClick={onKeep}>Keep this recommendation</Button><Button variant="soft" size="xl" className="w-full" onClick={onSwap}>Swap one thing</Button></div>}</section>;
}
...
function JourneyBar({ items, onOpen }: { items: { label: string; state: string; block: MonthBlock }[]; onOpen: (block: MonthBlock) => void }) {
  return <div className="absolute bottom-0 left-0 right-0 z-30 border-t border-border/80 bg-shell/95 px-3 py-2 backdrop-blur-xl"><div className="flex gap-2 overflow-x-auto pb-1">{items.map((item) => <button key={`${item.label}-${item.state}`} onClick={() => onOpen(item.block)} className="min-w-[78px] rounded-2xl bg-card px-2 py-2 text-center shadow-card focus-visible:outline-none focus-visible:ring-2 focus-visible:ring-ring"><span className={cn("mx-auto mb-1 block size-2 rounded-full", item.state === "completed" ? "bg-primary" : item.state === "selected" ? "bg-accent" : item.state === "needs input" ? "bg-rose" : item.state === "locked" ? "bg-muted-foreground" : "bg-secondary")} /><span className="block text-[10px] font-bold leading-tight">{item.label}</span><span className="mt-0.5 block text-[9px] capitalize text-muted-foreground">{item.state}</span></button>)}</div></div>;
}
...
function HomeScreen({ pathway, answers, onCare, onFuture, onJourney, onStamp }: { pathway: Pathway; answers: string[]; onCare: () => void; onFuture: () => void; onJourney: () => void; onStamp: (stamp: PassportStamp) => void }) {
  ...
  const key = pathwayKeyFromTitle(pathway);
  const nextUnlock: Record<PathwayKey, string> = { peri: "Smart Ring", endo: "PeriShield Rx", metabo: "DermaShield+", longevity: "Longevity Lab Rider" };
  ...
  const stack = [{ label: "Care", state: "Done" }, { label: "Coach", state: "Done" }, { label: "Pods", state: "Needs input" }, { label: "Pass", state: "Needs input" }, { label: "Kit", state: "Future" }, { label: "Unlocks", state: "Locked" }];
  return <section className="space-y-5 px-5 pb-28 pt-6"><div className="flex items-center justify-between"><div><p className="text-sm font-semibold text-accent">Stretch Journey Bar</p><h1 className="font-display text-4xl leading-tight">Command center</h1></div><button onClick={onCare} className="rounded-full bg-secondary p-3 text-accent shadow-card focus-visible:outline-none focus-visible:ring-2 focus-visible:ring-ring"><MessageCircle className="size-5" /></button></div><div className="rounded-[2rem] bg-primary p-6 text-primary-foreground shadow-float animate-enter"><div className="mb-5 flex items-center justify-between"><p className="font-display text-3xl">Today’s 3 actions</p><span className="rounded-full bg-primary-foreground/15 px-3 py-1 text-xs font-bold">Live</span></div>{["Build your kit", "Join your first pod", "Book your experience pass"].map((item, index) => <button key={item} onClick={onJourney} className="mt-3 flex w-full items-center justify-between rounded-2xl bg-primary-foreground/10 px-4 py-3 text-left font-semibold transition-smooth hover:bg-primary-foreground/15 focus-visible:outline-none focus-visible:ring-2 focus-visible:ring-primary-foreground/60"><span className="flex items-center gap-3"><span className="grid size-7 place-items-center rounded-full bg-primary-foreground text-xs font-bold text-primary">{index + 1}</span>{item}</span><ArrowRight className="size-4" /></button>)}</div><button onClick={onJourney} className="w-full rounded-[2rem] bg-card p-5 text-left shadow-card transition-smooth hover:-translate-y-0.5 focus-visible:outline-none focus-visible:ring-2 focus-visible:ring-ring"><div className="mb-4 flex items-center justify-between"><p className="font-display text-2xl">Your Month Stack</p><ArrowRight className="size-4 text-accent" /></div><div className="grid grid-cols-2 gap-2">{stack.map((item) => <div key={item.label} className="rounded-2xl bg-secondary p-3"><p className="text-sm font-bold">{item.label}</p><p className={cn("mt-1 text-xs font-semibold", item.state === "Done" ? "text-primary" : item.state === "Locked" ? "text-muted-foreground" : "text-accent")}>{item.state}</p></div>)}</div></button><div className="grid grid-cols-2 gap-3"><button onClick={onFuture} className="rounded-[2rem] bg-card p-5 text-left shadow-card transition-smooth hover:-translate-y-0.5 focus-visible:outline-none focus-visible:ring-2 focus-visible:ring-ring"><Lock className="mb-4 size-5 text-accent" /><p className="text-sm font-semibold text-muted-foreground">Next unlock</p><h2 className="mt-1 font-display text-2xl leading-tight">{nextUnlock[key]}</h2></button><div className="rounded-[2rem] bg-card p-5 shadow-card"><Package className="mb-4 size-5 text-accent" /><p className="text-sm font-semibold text-muted-foreground">Recommended now</p><h2 className="mt-1 font-display text-2xl leading-tight">{addOn[key]}</h2></div></div><MbcCommandCard onStamp={onStamp} /><div className="rounded-[2rem] bg-secondary p-5 shadow-card"><p className="font-display text-2xl">Why we recommended this</p><div className="mt-4 flex flex-wrap gap-2">{chips.map((chip) => <span key={chip} className="rounded-full bg-card px-3 py-2 text-xs font-semibold text-accent shadow-card">{chip}</span>)}</div><p className="mt-4 text-sm leading-6 text-muted-foreground">{pathway.reason}</p></div><Button variant="hero" size="xl" className="w-full" onClick={onJourney}>Continue building my month <ArrowRight className="size-4" /></Button></section>;
}
