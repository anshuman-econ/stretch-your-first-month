    ...
    <main className="min-h-screen bg-background font-sans text-foreground" onPointerMove={(event) => {
      ...
      <div className="relative mx-auto flex min-h-screen w-full max-w-md flex-col overflow-hidden bg-shell shadow-shell sm:my-6 sm:min-h-[860px] sm:rounded-[2rem]">
        ...
        <div className="flex-1 overflow-y-auto pb-24">
          ...
          {step === "explainer" && <ExplainerScreen onContinue={() => setStep("quiz")} onExample={() => { setGoal("Sleep through the night"); setAnswers([]); setSelectedJourney("peri"); setStep("builder"); }} onBlocks={() => setShowBlocksDemo(true)} />}
...
function ExplainerScreen({ onContinue, onExample, onBlocks }: { onContinue: () => void; onExample: () => void; onBlocks: () => void }) {
  return <section className="space-y-7 px-5 py-7"><div className="rounded-[2rem] bg-hero p-6 shadow-float animate-enter"><div className="flex items-start justify-between gap-4"><div><h1 className="font-display text-4xl leading-tight">What’s Stretch?</h1><p className="mt-3 text-base leading-7 text-muted-foreground">Stretch turns what you want to feel better into a guided first month — then helps you use it, earn credits, and unlock the next step.</p></div><Sparkles className="size-7 shrink-0 text-accent" /></div><div className="mt-5 grid gap-3"><Button variant="hero" size="xl" onClick={onContinue}>Take the quiz <ArrowRight className="size-4" /></Button><Button variant="soft" size="xl" onClick={onExample}>Show me an example month</Button></div></div><ExplainerSection title="How Stretch works"><div className="grid gap-3"><VisualStep number="1" title="Tell us what should feel better" copy="Start with the one thing you want to improve in the next 30 days."><div className="flex flex-wrap gap-2">{["sleep", "energy", "pain", "glow", "focus"].map((item) => <span key={item} className="rounded-full bg-secondary px-3 py-2 text-xs font-bold text-accent">{item}</span>)}</div></VisualStep><VisualStep number="2" title="Stretch builds your month" copy="You get a recommended care route, coach plan, pods, kit, and experience pass — with safe swaps if one piece needs adjusting."><div className="grid grid-cols-3 gap-2">{["Care", "Coach", "Kit"].map((item) => <span key={item} className="rounded-2xl bg-secondary px-3 py-3 text-center text-xs font-bold text-accent">{item}</span>)}</div></VisualStep><VisualStep number="3" title="Use it, earn credits, unlock next steps" copy="Complete the actions that make your plan work, earn Milestone Bonus Credits, and open the next layer only when it’s useful."><div className="h-2 rounded-full bg-secondary"><div className="h-full w-2/3 rounded-full bg-primary" /></div></VisualStep></div></ExplainerSection><ExplainerSection title="Why Stretch is different"><div className="grid gap-3"><CompareCard title="Static programs" copy="One fixed bundle, even if your needs change." muted /><CompareCard title="Open marketplaces" copy="Too many disconnected choices to manage yourself." muted /><CompareCard title="Stretch" copy="A guided month, one safe swap, and coach rebalancing when needed." highlight /></div></ExplainerSection><div className="rounded-[2rem] bg-primary p-6 text-primary-foreground shadow-float"><h2 className="font-display text-3xl leading-tight">Ready to build your first month?</h2><div className="mt-5 grid gap-3"><Button variant="soft" size="xl" onClick={onContinue}>Start quiz</Button><Button variant="soft" size="xl" onClick={onExample}>Show me an example plan</Button></div></div><div className="border-t border-border pt-5 text-center"><p className="text-xs font-bold uppercase tracking-wide text-muted-foreground">For demo / investors</p><button onClick={onBlocks} className="mt-2 text-sm font-bold text-accent underline-offset-4 hover:underline focus-visible:outline-none focus-visible:ring-2 focus-visible:ring-ring">View the full platform blocks</button></div></section>;
}
