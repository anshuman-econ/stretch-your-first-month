        ...
        <div className="flex-1 overflow-y-auto pb-24">
          {step === "landing" && (
            <section className="space-y-6 px-5 pb-8 pt-6">
              ...
              <button onClick={() => setStep("pathways")} className="w-full rounded-[2rem] bg-card p-5 text-left shadow-card transition-smooth hover:-translate-y-0.5 focus-visible:outline-none focus-visible:ring-2 focus-visible:ring-ring"><div className="flex items-start gap-4"><div className="rounded-2xl bg-secondary p-3 text-accent"><Sparkles className="size-5" /></div><div className="min-w-0 flex-1"><div className="flex items-start justify-between gap-3"><p className="font-display text-2xl leading-tight">Pathways ready</p><span className="rounded-full bg-secondary px-3 py-1 text-xs font-bold text-accent">4 open</span></div><p className="mt-2 text-sm text-muted-foreground">Start with one guided month.</p><p className="mt-4 text-sm font-bold text-accent">Learn more</p></div></div></button>
              <button onClick={() => setStep("future")} className="w-full rounded-[2rem] bg-secondary p-5 text-left shadow-card transition-smooth hover:-translate-y-0.5 focus-visible:outline-none focus-visible:ring-2 focus-visible:ring-ring"><div className="flex items-start gap-4"><div className="rounded-2xl bg-card p-3 text-accent shadow-card"><Lock className="size-5" /></div><div className="min-w-0 flex-1"><div className="flex items-start justify-between gap-3"><p className="font-display text-2xl leading-tight">Future routes</p><span className="rounded-full bg-card px-3 py-1 text-xs font-bold text-muted-foreground shadow-card">Locked</span></div><p className="mt-2 text-sm text-muted-foreground">Preview what may open later.</p><p className="mt-4 text-sm font-bold text-accent">Learn more</p></div></div></button>
...
function GoalScreen({ setGoal, setStep }: { setGoal: (goal: string) => void; setStep: (step: Step) => void }) {
  return <section className="space-y-6 px-5 py-7"><SectionTitle title="What should feel better?" copy="Pick one focus for the next 30 days." /><div className="grid gap-3">{goals.map((item) => <SoftCard key={item} onClick={() => { setGoal(item); setStep("quiz"); }} className="p-4"><span className="flex items-center justify-between gap-3 font-medium"><span>{item}</span><ArrowRight className="size-4 text-accent" /></span></SoftCard>)}</div></section>;
}
...
function BuiltScreen({ pathway, resetQuiz, onUnlocks, onSwap, onCoach }: { pathway: Pathway; resetQuiz: () => void; onUnlocks: () => void; onSwap: () => void; onCoach: () => void }) {
  return <section className="space-y-6 px-5 py-7"><div className="rounded-[2rem] bg-primary p-6 text-primary-foreground shadow-float"><Sparkles className="mb-5 size-8" /><SectionTitle title="Stretch built your month." copy="Keep it, swap one thing, or ask your coach." /></div><div className="rounded-[2rem] bg-card p-5 shadow-card"><div className="flex items-start gap-4"><div className="rounded-2xl bg-secondary p-3 text-accent"><Sparkles className="size-5" /></div><div className="min-w-0 flex-1"><div className="flex items-start justify-between gap-3"><p className="font-display text-2xl leading-tight">{pathway.title}</p><span className="rounded-full bg-secondary px-3 py-1 text-xs font-bold text-accent">Ready</span></div><p className="mt-2 text-sm text-muted-foreground">Your first guided stack is ready.</p><button className="mt-4 text-sm font-bold text-accent" onClick={onUnlocks}>Learn more</button></div></div></div><div className="grid gap-3"><Button variant="hero" size="xl" onClick={onUnlocks}>Show my unlocks</Button><div className="grid grid-cols-2 gap-2"><Button variant="soft" size="lg" onClick={onSwap}>Swap one thing</Button><Button variant="soft" size="lg" onClick={onCoach}>Ask coach</Button></div><Button variant="soft" size="lg" onClick={resetQuiz}>Change answers</Button></div></section>;
}
...
function SwapScreen({ pathway, reason, onBack, onCoach }: { pathway: Pathway; reason: string; onBack: () => void; onCoach: () => void }) {
  ...
  return <section className="space-y-5 px-5 py-7"><div className="rounded-[2rem] bg-hero p-6 shadow-float"><p className="text-sm font-semibold text-accent">Controlled swap</p><h1 className="mt-1 font-display text-4xl leading-tight">Choose one block</h1><p className="mt-3 rounded-2xl bg-card/80 p-3 text-sm font-semibold text-muted-foreground shadow-card">One swap this month. Bigger changes go to coach.</p></div><div className="grid gap-3">{swaps.map((swap) => <button key={swap.name} onClick={() => setSelected(swap)} className="rounded-[2rem] bg-card p-5 text-left shadow-card transition-smooth hover:-translate-y-0.5 focus-visible:outline-none focus-visible:ring-2 focus-visible:ring-ring"><div className="flex items-start gap-4"><div className="rounded-2xl bg-secondary p-3 text-accent">{iconForBlock(swap.name)}</div><div className="min-w-0 flex-1"><div className="flex items-start justify-between gap-3"><p className="font-display text-2xl leading-tight">{swap.name}</p><span className="rounded-full bg-secondary px-3 py-1 text-xs font-bold text-accent">Swappable</span></div><p className="mt-2 text-sm text-muted-foreground">One guided alternative.</p><p className="mt-4 text-sm font-bold text-accent">Learn more</p></div></div></button>)}</div><Button variant="soft" size="xl" className="w-full" onClick={onBack}>Back to month</Button>{selected && <SwapDrawer swap={selected} onClose={() => setSelected(null)} onCoach={onCoach} />}</section>;
}
...
function BlockDrawer({ block, pathway, onClose, onCoach }: { block: MonthBlock; pathway: Pathway; onClose: () => void; onCoach: () => void }) {
  ...
  return <div className="absolute inset-0 z-50 flex items-end bg-primary/25 p-3 backdrop-blur-sm" onClick={onClose}><div className="max-h-[82vh] w-full overflow-y-auto rounded-[2rem] bg-card p-6 shadow-float animate-slide-up" onClick={(event) => event.stopPropagation()}><div className="mb-5 flex items-start justify-between gap-3"><div className="flex items-start gap-3"><div className="rounded-full bg-secondary p-3 text-accent">{iconForBlock(block.name)}</div><div><p className="text-sm font-semibold text-accent">{block.name}</p><h2 className="font-display text-3xl leading-tight">{block.selection}</h2></div></div><button onClick={onClose} className="rounded-full bg-secondary px-3 py-2 text-sm font-semibold text-accent">Close</button></div><div className="grid gap-3"><InfoBlock label="Why here" copy={block.why} /><InfoBlock label="Inside" copy={block.includes} /><div className="rounded-2xl bg-secondary p-4"><p className="text-xs font-semibold uppercase tracking-wide text-accent">One recommendation</p><div className="mt-3 grid gap-2">{catalog.options.slice(0, 1).map((option) => <div key={option.name} className="rounded-2xl bg-card px-4 py-3 shadow-card"><p className="text-sm font-semibold">{option.name}</p><p className="mt-1 text-xs text-accent">{option.state}</p></div>)}</div>{catalog.agenda && <p className="mt-3 text-xs leading-5 text-muted-foreground">{catalog.agenda}</p>}</div></div><div className="mt-5 grid gap-3"><Button variant="hero" size="xl" onClick={onClose}>Keep this piece</Button><Button variant="soft" size="xl" disabled={!block.swappable} onClick={onClose}><RefreshCw className="size-4" /> Preview one swap</Button><Button variant="soft" size="xl" onClick={onCoach}>Ask coach</Button></div></div></div>;
}
