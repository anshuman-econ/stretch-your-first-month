...
function UnlocksScreen({ pathway, answers, onBuild, onKeep, onSwap }: { pathway: Pathway; answers: string[]; onBuild: () => void; onKeep: () => void; onSwap: () => void }) {
  ...
  return <section className="space-y-6 px-5 py-7"><div className="space-y-2"><h1 className="font-display text-4xl leading-tight">Your Stretch month is ready</h1><p className="text-base leading-7 text-muted-foreground">Here’s what unlocked for you.</p></div><div className="flex snap-x gap-4 overflow-x-auto pb-3"><div className="w-2 shrink-0" />{cards.map((card, index) => <div key={card.title} className="min-h-[330px] w-[82%] shrink-0 snap-center rounded-[2rem] bg-card p-5 shadow-float animate-enter" style={{ animationDelay: `${index * 90}ms` }}><div className="flex h-full flex-col justify-between"><div><div className="mb-5 flex items-center justify-between gap-3"><div className="rounded-full bg-secondary p-3 text-accent shadow-card">{card.icon}</div><span className="rounded-full bg-secondary px-3 py-1 text-xs font-bold text-accent">{card.status}</span></div><p className="text-sm font-bold text-accent">{card.title}</p><h2 className="mt-2 font-display text-3xl leading-tight">{card.lead}</h2><p className="mt-3 text-sm leading-6 text-muted-foreground">{card.copy}</p></div><button className="mt-6 inline-flex items-center justify-between rounded-2xl bg-hero px-4 py-3 text-sm font-bold text-accent shadow-card">Swipe / continue <ArrowRight className="size-4" /></button></div></div>)}</div><div className="grid gap-3"><Button variant="hero" size="xl" className="w-full" onClick={onBuild}>Build my month</Button><Button variant="soft" size="xl" className="w-full" onClick={onKeep}>Keep this recommendation</Button><Button variant="soft" size="xl" className="w-full" onClick={onSwap}>Swap one thing</Button></div></section>;
}
