...
function UnlocksScreen({ pathway, answers, onBuild, onKeep, onSwap }: { pathway: Pathway; answers: string[]; onBuild: () => void; onKeep: () => void; onSwap: () => void }) {
  ...
  const card = cards[activeUnlockIndex];
  const continueReveal = () => activeUnlockIndex === cards.length - 1 ? setComplete(true) : setActiveUnlockIndex((current) => current + 1);
  return <section className="flex min-h-full flex-col px-5 py-7"><div className="space-y-2"><p className="text-sm font-bold text-accent">Membership activation</p><h1 className="font-display text-4xl leading-tight">Your Stretch month is ready</h1><p className="text-base leading-7 text-muted-foreground">We’re opening each part of your first month in order.</p></div><div className="mt-6 flex flex-1 flex-col justify-center"><div key={card.title} className="min-h-[420px] rounded-[2rem] bg-card p-6 shadow-float animate-enter"><div className="flex h-full min-h-[372px] flex-col justify-between"><div><div className="mb-6 flex items-center justify-between gap-3"><div className="rounded-full bg-secondary p-4 text-accent shadow-card">{card.icon}</div><span className="rounded-full bg-secondary px-3 py-1 text-xs font-bold text-accent">{card.status}</span></div><p className="text-sm font-bold text-accent">{card.title}</p><h2 className="mt-3 font-display text-4xl leading-tight">{card.lead}</h2><p className="mt-4 text-base leading-7 text-muted-foreground">{card.copy}</p></div><Button variant="hero" size="xl" className="mt-8 w-full" onClick={continueReveal}>{activeUnlockIndex === cards.length - 1 ? "Finish activation" : "Continue"} <ArrowRight className="size-4" /></Button></div></div><p className="mt-5 text-center text-sm font-bold text-accent">Step {complete ? cards.length : activeUnlockIndex + 1} of {cards.length}</p></div>{complete && <div className="mt-6 grid gap-3 animate-slide-up"><Button variant="hero" size="xl" className="w-full" onClick={onBuild}>Build my month</Button><Button variant="soft" size="xl" className="w-full" onClick={onKeep}>Keep this recommendation</Button><Button variant="soft" size="xl" className="w-full" onClick={onSwap}>Swap one thing</Button></div>}</section>;
}
