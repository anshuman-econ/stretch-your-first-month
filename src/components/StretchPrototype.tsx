...
function BuilderScreen({ pathway, onStart, onCoach }: { pathway: Pathway; onStart: () => void; onCoach: () => void }) {
  ...
  return <section className="space-y-6 px-5 py-7"><div className="rounded-[2rem] bg-hero p-6 shadow-float"><p className="mb-3 text-sm font-semibold text-accent">Guided monthly care stack</p><SectionTitle title="We built your month." copy="Your plan is already assembled. Explore each piece, then keep it or ask your coach from the details." /><button onClick={() => setShowBlocks(true)} className="mt-5 inline-flex items-center gap-2 rounded-full bg-card px-4 py-3 text-sm font-semibold text-accent shadow-card"><Sparkles className="size-4" /> View all building blocks</button></div><div className="rounded-3xl bg-card p-5 shadow-card"><p className="font-display text-2xl">Build My Month steps</p><div className="mt-4 grid gap-3">{steps.map((step, index) => <button key={step} onClick={() => setDrawerBlock(stack[Math.min(index === 0 ? 0 : index + 1, stack.length - 1)])} className="flex items-center gap-3 rounded-2xl bg-secondary px-3 py-3 text-left transition-smooth hover:-translate-y-0.5"><span className="flex size-8 items-center justify-center rounded-full bg-card text-xs font-bold text-accent shadow-card">{index + 1}</span><span className="font-medium">{step}</span><ArrowRight className="ml-auto size-4 text-accent" /></button>)}</div></div><div className="grid grid-cols-3 gap-2 rounded-3xl bg-card p-2 shadow-card">{[["keep", "Explore"], ["swap", "Swaps"], ["coach", "Coach"]].map(([key, label]) => <button key={key} onClick={() => setMode(key as "keep" | "swap" | "coach")} className={cn("rounded-2xl px-2 py-3 text-xs font-semibold transition-smooth focus-visible:outline-none focus-visible:ring-2 focus-visible:ring-ring", mode === key ? "bg-primary text-primary-foreground" : "text-muted-foreground hover:bg-secondary")}>{label}</button>)}</div>{mode === "coach" && <div className="rounded-3xl bg-secondary p-5 shadow-card"><p className="font-display text-2xl">Ask your coach to rebalance</p><p className="mt-2 text-sm leading-6 text-muted-foreground">Pick the direction that would make this month feel easier. The core care route stays safe.</p><div className="mt-4 grid gap-2">{rebalanceOptions.map((option) => <button key={option} onClick={() => setRebalance(option)} className={cn("rounded-2xl px-4 py-3 text-left text-sm font-semibold transition-smooth", rebalance === option ? "bg-primary text-primary-foreground" : "bg-card text-foreground shadow-card")}>{option}</button>)}</div>{updated.length > 0 && <div className="mt-5 rounded-3xl bg-card p-4 shadow-card"><p className="font-semibold">What your coach would adjust</p><div className="mt-3 grid gap-2">{updated.map((item) => <p key={item} className="flex gap-2 text-sm leading-6 text-muted-foreground"><Check className="mt-1 size-4 shrink-0 text-accent" />{item}</p>)}</div></div>}</div>}<div className="space-y-3"><p className="font-display text-3xl">Discover your plan</p>{stack.map((block, index) => <SoftCard key={block.name} onClick={() => setDrawerBlock(block)} className={cn("space-y-3", mode === "swap" && block.swappable && "border-accent/60")}><div className="flex items-start gap-3"><div className="rounded-full bg-secondary p-3 text-accent">{iconForBlock(block.name)}</div><div className="min-w-0 flex-1"><div className="flex items-start justify-between gap-3"><p className="text-sm font-semibold text-accent">{block.name}</p><span className="shrink-0 rounded-full bg-secondary px-3 py-1 text-xs font-semibold text-accent">{block.status}</span></div><h2 className="mt-1 font-display text-2xl leading-tight">{block.selection}</h2></div></div><p className="text-sm leading-6 text-muted-foreground">{block.plain}</p><p className="flex items-center gap-2 text-xs font-semibold text-accent">Tap for details <ArrowRight className="size-3" /></p></SoftCard>)}</div><Button variant="hero" size="xl" className="w-full" onClick={onStart}>Start week one</Button>{drawerBlock && <BlockDrawer block={drawerBlock} pathway={pathway} mode={mode} onClose={() => setDrawerBlock(null)} onCoach={onCoach} />}{showBlocks && <BlocksDemoDrawer onClose={() => setShowBlocks(false)} onTile={setDemoTile} />}{demoTile && <DemoTileDrawer tile={demoTile} onClose={() => setDemoTile(null)} />}</section>;
}
...
function BlockDrawer({ block, pathway, mode, onClose, onCoach }: { block: MonthBlock; pathway: Pathway; mode: "keep" | "swap" | "coach"; onClose: () => void; onCoach: () => void }) {
  ...
  return <div className="absolute inset-0 z-50 flex items-end bg-primary/25 p-3 backdrop-blur-sm" onClick={onClose}><div className="max-h-[88vh] w-full overflow-y-auto rounded-[2rem] bg-card p-6 shadow-float" onClick={(event) => event.stopPropagation()}><div className="mb-5 flex items-start justify-between gap-3"><div className="flex items-start gap-3"><div className="rounded-full bg-secondary p-3 text-accent">{iconForBlock(block.name)}</div><div><p className="text-sm font-semibold text-accent">{block.name}</p><h2 className="font-display text-3xl leading-tight">{block.selection}</h2></div></div><button onClick={onClose} className="rounded-full bg-secondary px-3 py-2 text-sm font-semibold text-accent">Close</button></div><div className="grid gap-4"><InfoBlock label="What this does" copy={block.plain} /><InfoBlock label="Why it is in your month" copy={block.why} /><InfoBlock label="Discovery moment" copy={block.discovery} /><InfoBlock label="What is included" copy={block.includes} /><div className="rounded-2xl bg-secondary p-4"><p className="text-xs font-semibold uppercase tracking-wide text-accent">What you can explore</p><p className="mt-2 text-sm leading-6 text-muted-foreground">{catalog.rule}</p><div className="mt-3 grid gap-2">{catalog.options.map((option) => <div key={option.name} className="rounded-2xl bg-card px-4 py-3 shadow-card"><p className="text-sm font-semibold">{option.name}</p><p className="mt-1 text-xs text-accent">{option.state}</p></div>)}</div>{catalog.agenda && <p className="mt-3 text-xs leading-5 text-muted-foreground">{catalog.agenda}</p>}</div><InfoBlock label="If you swap this" copy={block.swappable ? block.change || "Only this guided piece changes. Your pathway, coach support, and safety boundaries stay in place." : "This piece stays guided because it affects safety, sequencing, or future unlocks."} /></div><div className="mt-5 grid gap-3"><Button variant="hero" size="xl" onClick={onClose}>Keep this piece</Button><Button variant="soft" size="xl" disabled={!block.swappable || mode !== "swap"} onClick={onClose}><RefreshCw className="size-4" /> Preview swap</Button><Button variant="soft" size="xl" onClick={onCoach}>Ask coach</Button></div></div></div>;
}
