...
export default function StretchPrototype() {
  ...
  const [showWhatsStretch, setShowWhatsStretch] = useState(false);
  ...
  const openWhatsStretch = (returnStep: Step) => {
    setExplainerReturnStep(returnStep);
    setShowWhatsStretch(true);
  };
  const continueFromExplainer = () => {
    setShowWhatsStretch(false);
    ...
    <main className="min-h-screen bg-background font-sans text-foreground" onPointerMove={(event) => {
      ...
      <div className="relative mx-auto flex min-h-screen w-full max-w-md flex-col overflow-hidden bg-shell shadow-shell sm:my-6 sm:min-h-[860px] sm:rounded-[2rem]">
        ...
        <div className="flex-1 overflow-y-auto pb-24">
              ...
              <div className="relative overflow-hidden rounded-[2rem] bg-hero p-6 shadow-float">
                ...
                <div className="relative flex min-h-[520px] flex-col justify-between">
                  ...
                  <div className="space-y-5">
                    ...
                    <Button variant="hero" size="xl" className="w-full" onClick={() => setStep("goal")}>Answer a few questions <ArrowRight className="size-4" /></Button><button onClick={() => openWhatsStretch("landing")} aria-label="Learn what Stretch is" className="mx-auto inline-flex items-center gap-2 rounded-full border border-border bg-card/85 px-4 py-2 text-sm font-bold text-accent shadow-card backdrop-blur transition-smooth hover:-translate-y-0.5 focus-visible:outline-none focus-visible:ring-2 focus-visible:ring-ring"><Sparkles className="size-4" /> New here? What’s Stretch?</button>
                  </div>
                </div>
              </div>
          ...
          {step === "goal" && <GoalScreen setGoal={setGoal} setStep={setStep} onExplain={() => openWhatsStretch("goal")} />}
          {step === "quiz" && <QuizScreen quizIndex={quizIndex} chooseAnswer={chooseAnswer} onExplain={() => openWhatsStretch("quiz")} />}
        ...
        {showWhatsStretch && <ExplainerScreen onClose={() => setShowWhatsStretch(false)} onContinue={continueFromExplainer} onExample={() => setShowExampleMonthPreview(true)} />}
        {showExampleMonthPreview && <ExampleMonthPreview onClose={() => setShowExampleMonthPreview(false)} onStartQuiz={() => { setShowExampleMonthPreview(false); setShowWhatsStretch(false); if (goal) setStep("quiz"); else setStep("goal"); }} />}
...
function GoalScreen({ setGoal, setStep, onExplain }: { setGoal: (goal: string) => void; setStep: (step: Step) => void; onExplain: () => void }) {
  return <section className="space-y-6 px-5 py-7"><div className="flex justify-end"><button onClick={onExplain} aria-label="Learn what Stretch is" className="inline-flex items-center gap-2 rounded-full border border-border bg-card px-3 py-2 text-xs font-bold text-accent shadow-card transition-smooth hover:-translate-y-0.5 focus-visible:outline-none focus-visible:ring-2 focus-visible:ring-ring"><Sparkles className="size-3.5" /> What’s Stretch?</button></div><SectionTitle title="What do you want to feel better in 30 days?" copy="Choose the answer that feels closest. Your first month can still be adjusted." /><div className="grid gap-3">{goals.map((item) => <SoftCard key={item} onClick={() => { setGoal(item); setStep("quiz"); }} className="p-4"><span className="flex items-center justify-between gap-3 font-medium"><span>{item}</span><ArrowRight className="size-4 text-accent" /></span></SoftCard>)}</div></section>;
}

type ExplainerDetail = { title: string; status?: string; copy: string; examples?: string[] };

function ExplainerScreen({ onContinue, onClose, onExample }: { onContinue: () => void; onClose: () => void; onExample: () => void }) {
  ...
  const chipStatus = (chip: string) => chip.includes("riders") ? "rider-only" : chip.includes("ring") || chip.includes("CGM") || chip.includes("home lab") ? "future unlock" : chip.includes("LED face") || chip.includes("glow") ? "pack-only" : chip.includes("magnesium") || chip.includes("collagen") || chip.includes("smart snack") ? "swappable" : "included now";
  const chipCopy = (chip: string) => ({ title: chip, status: chipStatus(chip), copy: `${chip} can appear when it matches your pathway, progress, availability, and what is safe for this month. It is not automatically included for every member.` });
  return <section className="absolute inset-0 z-50 flex flex-col bg-shell p-4 pb-6"><div className="mb-4 flex items-center justify-between gap-3"><div><p className="text-sm font-bold text-accent">Member explainer</p><h1 className="font-display text-3xl leading-tight">What’s Stretch?</h1></div><button onClick={onClose} className="rounded-full bg-secondary px-4 py-2 text-sm font-bold text-accent shadow-card focus-visible:outline-none focus-visible:ring-2 focus-visible:ring-ring">Close</button></div><div className="flex-1 space-y-5 overflow-y-auto pr-1"><div className="rounded-[2rem] bg-hero p-5 shadow-float"><h2 className="font-display text-3xl leading-tight">Stretch builds your first month.</h2><p className="mt-3 text-sm leading-6 text-muted-foreground">Tell us what you want to feel better in 30 days. Stretch builds a guided month with care, coaching, pods, a kit, an experience pass, and future unlocks.</p><div className="mt-5 grid grid-cols-2 gap-2">{["Care", "Coach", "Labs", "Pods", "Pass", "Kit"].map((item, index) => <div key={item} className="flex items-center justify-between rounded-2xl bg-card/90 px-4 py-3 text-sm font-bold shadow-card"><span>{item}</span><span className="rounded-full bg-secondary px-2 py-1 text-[10px] text-accent">0{index + 1}</span></div>)}</div><div className="mt-5 grid gap-3"><Button variant="hero" size="lg" onClick={onContinue}>Start quiz</Button><Button variant="soft" size="lg" onClick={onExample}>See example month</Button></div></div><ExplainerSection title="Why it’s different"><div className="grid gap-3"><CompareCard title="Static programs" copy="One fixed bundle, even when your symptoms change." muted /><CompareCard title="Open marketplaces" copy="Lots of disconnected options, but you still have to figure out what to choose." muted /><CompareCard title="Stretch" copy="A guided month built for you. Keep it, swap one thing, or ask your coach." highlight /></div></ExplainerSection><div className="overflow-hidden rounded-[2rem] bg-card p-5 shadow-card"><p className="mb-4 font-display text-2xl leading-tight">A wellness-first subscription that can include or unlock…</p><div className="group flex gap-2 overflow-hidden"><div className="flex shrink-0 animate-marquee gap-2 group-hover:[animation-play-state:paused]">{[...chips, ...chips].map((chip, index) => <button key={`${chip}-${index}`} onClick={() => setDetail(chipCopy(chip))} className="shrink-0 rounded-full bg-secondary px-4 py-2 text-xs font-bold text-accent shadow-card focus-visible:outline-none focus-visible:ring-2 focus-visible:ring-ring">{chip}</button>)}</div></div></div><div className="rounded-[2rem] bg-card p-5 shadow-card"><h2 className="font-display text-3xl leading-tight">What are Milestone Bonus Credits?</h2><p className="mt-3 text-sm leading-6 text-muted-foreground">Milestone Bonus Credits are Stretch-funded credits you earn by completing the steps that make your plan work — like building your kit, joining a pod, completing labs, booking your pass, or keeping a 7-day streak.</p><div className="mt-4 grid gap-3"><div className="rounded-2xl bg-secondary p-4"><p className="text-xs font-bold uppercase tracking-wide text-accent">Earn with</p><div className="mt-3 flex flex-wrap gap-2">{["build your kit", "join your pod", "complete your lab", "book your experience pass", "finish coaching", "keep a 7-day tracking streak"].map((item) => <span key={item} className="rounded-full bg-card px-3 py-2 text-xs font-bold text-accent shadow-card">{item}</span>)}</div></div><div className="rounded-2xl bg-secondary p-4"><p className="text-xs font-bold uppercase tracking-wide text-accent">Use for</p><div className="mt-3 grid grid-cols-2 gap-2">{["selected packs", "device buy-downs", "kit upgrades", "future pathway add-ons", "special partner experiences"].map((item) => <span key={item} className="rounded-2xl bg-card px-3 py-3 text-xs font-bold text-muted-foreground shadow-card">{item}</span>)}</div></div></div><p className="mt-4 rounded-2xl bg-sand p-3 text-xs font-bold leading-5 text-sand-foreground">MBC is not insurance cashback. It is a Stretch reward that helps you unlock the next layer of support.</p></div></div><div className="mt-4 grid gap-2"><Button variant="hero" size="lg" onClick={onContinue}>Start quiz</Button><Button variant="soft" size="lg" onClick={onExample}>Show example month</Button><Button variant="soft" size="lg" onClick={onClose}>Close</Button></div>{detail && <ExplainerDetailDrawer detail={detail} onClose={() => setDetail(null)} />}</section>;
}

function ExampleMonthPreview({ onClose, onStartQuiz }: { onClose: () => void; onStartQuiz: () => void }) {
  const cards = [
    ["Care", "Peri-aware review"],
    ["Coach", "Sleep routine + symptom routine"],
    ["Pods", "Peri Reset + Sleep Reset"],
    ["Pass", "Cooling / breathwork reset"],
    ["Kit", "Magnesium + sleep kit + visible vitality support"],
    ["Unlocks", "Smart Ring + PeriShield preview"],
  ];
  return <div className="absolute inset-0 z-[60] flex items-end bg-primary/25 p-3 backdrop-blur-sm" onClick={onClose}><div className="max-h-[86vh] w-full overflow-y-auto rounded-[2rem] bg-card p-6 shadow-float animate-slide-up" onClick={(event) => event.stopPropagation()}><div className="mb-5 flex items-start justify-between gap-3"><div><p className="text-sm font-bold text-accent">Example Stretch Month</p><h2 className="font-display text-3xl leading-tight">Peri Sleep + Energy</h2></div><button onClick={onClose} className="rounded-full bg-secondary px-3 py-2 text-sm font-semibold text-accent">Close</button></div><div className="grid gap-3">{cards.map(([title, copy]) => <div key={title} className="rounded-2xl bg-secondary p-4"><p className="text-xs font-bold uppercase tracking-wide text-accent">{title}</p><p className="mt-2 font-display text-xl leading-tight">{copy}</p></div>)}</div><div className="mt-5 grid gap-3"><Button variant="soft" size="xl" onClick={onClose}>Close preview</Button><Button variant="hero" size="xl" onClick={onStartQuiz}>Start quiz</Button></div></div></div>;
}
