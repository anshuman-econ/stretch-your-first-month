...
type ActivationDetail = { title: string; eyebrow: string; copy: string; rows?: { label: string; copy: string }[]; chips?: string[]; options?: CatalogOption[]; selectedOption?: string; onSelect?: (name: string) => void; action?: string };
...
function ActivationDetailDrawer({ detail, onClose }: { detail: ActivationDetail; onClose: () => void }) {
  return <div className="absolute inset-0 z-50 flex items-end bg-primary/25 p-3 backdrop-blur-sm" onClick={onClose}><div className="max-h-[84vh] w-full overflow-y-auto rounded-[2rem] bg-card p-6 shadow-float animate-slide-up" onClick={(event) => event.stopPropagation()}><div className="mb-5 flex items-start justify-between gap-3"><div><p className="text-sm font-bold text-accent">{detail.eyebrow}</p><h2 className="font-display text-3xl leading-tight">{detail.title}</h2>{detail.selectedOption && <p className="mt-1 text-xs font-bold text-accent">Selected: {detail.selectedOption}</p>}</div><button onClick={onClose} className="rounded-full bg-secondary px-3 py-2 text-sm font-semibold text-accent focus-visible:outline-none focus-visible:ring-2 focus-visible:ring-ring">Close</button></div><p className="text-sm leading-6 text-muted-foreground">{detail.copy}</p>{detail.rows && <div className="mt-4 grid gap-3">{detail.rows.map((row) => <InfoBlock key={row.label} label={row.label} copy={row.copy} />)}</div>}{detail.chips && <div className="mt-4 flex flex-wrap gap-2">{detail.chips.map((chip) => <span key={chip} className="rounded-full bg-secondary px-3 py-2 text-xs font-bold text-accent shadow-card">{chip}</span>)}</div>}{detail.options && <div className="mt-4 grid gap-2">{detail.options.map((option) => { const selected = detail.selectedOption === option.name; return <button key={option.name} onClick={() => detail.onSelect?.(option.name)} className={cn("rounded-2xl px-4 py-3 text-left shadow-card transition-smooth hover:-translate-y-0.5 focus-visible:outline-none focus-visible:ring-2 focus-visible:ring-ring", selected ? "bg-primary text-primary-foreground" : "bg-secondary")}><span className="flex items-start justify-between gap-3"><span><p className="text-sm font-semibold">{option.name}</p>{option.state && <p className={cn("mt-1 text-xs font-bold", selected ? "text-primary-foreground/80" : "text-accent")}>{selected ? "Selected" : option.state}</p>}</span>{selected && <Check className="mt-0.5 size-4 shrink-0" />}</span></button>; })}</div>}{detail.action && <Button variant="hero" size="lg" className="mt-5 w-full" onClick={onClose}>{detail.action}</Button>}</div></div>;
}
...
function BlueprintDrawer({ title, block, pathway, selectedOption, onSelect, onClose, onSwap }: { title: BlueprintTitle; block: MonthBlock; pathway: Pathway; selectedOption?: string; onSelect: (name: string) => void; onClose: () => void; onSwap: (target?: string) => void }) {
  ...
  const actions = sections.find((section) => section.label === "Actions")?.items || [];
  const actionHandler = (action: string) => action.toLowerCase().includes("swap") ? onSwap(swapTargetForBlueprint(title)) : onClose();
  return <div className="absolute inset-0 z-50 flex items-end bg-primary/25 p-3 backdrop-blur-sm" onClick={onClose}><div className="max-h-[84vh] w-full overflow-y-auto rounded-[2rem] bg-card p-6 shadow-float animate-slide-up" onClick={(event) => event.stopPropagation()}><div className="mb-5 flex items-start justify-between gap-3"><div className="flex items-start gap-3"><div className="rounded-full bg-secondary p-3 text-accent">{iconForBlock(title)}</div><div><p className="text-sm font-bold text-accent">Blueprint detail</p><h2 className="font-display text-3xl leading-tight">{title}</h2>{selectedOption && <p className="mt-1 text-xs font-bold text-accent">Selected: {selectedOption}</p>}</div></div><button onClick={onClose} className="rounded-full bg-secondary px-3 py-2 text-sm font-semibold text-accent focus-visible:outline-none focus-visible:ring-2 focus-visible:ring-ring">Close</button></div><div className="grid gap-3">{sections.filter((section) => section.label !== "Actions").map((section) => <BlueprintSectionBlock key={section.label} section={section} selectedOption={selectedOption} onSelect={onSelect} />)}</div><div className="mt-5 grid gap-3">{actions.map((action, index) => <Button key={action.name} variant={index === 0 ? "hero" : "soft"} size="xl" onClick={() => actionHandler(action.name)}>{action.name}</Button>)}</div></div></div>;
}

function BuiltScreen({ pathway, resetQuiz, onUnlocks, onSwap }: { pathway: Pathway; resetQuiz: () => void; onUnlocks: () => void; onSwap: (target?: string) => void }) {
...
function UnlocksScreen({ pathway, onBuild, onKeep, onSwap }: { pathway: Pathway; answers: string[]; onBuild: () => void; onKeep: () => void; onSwap: () => void }) {
  ...
  const passOptions = splitBlueprintList(activation.passes).map((name, index) => ({ name, state: index === 0 ? "Recommended" : /LED|Biopeak|red-light|clinic/i.test(name) ? "inventory-gated" : "Selectable" }));
  const functionalOptions = splitBlueprintList(activation.functional).map((name, index) => ({ name, state: index === 0 ? "Recommended" : "Selectable" }));
  const kitOptions = kitCatalog[key].map((name, index) => ({ name, state: index === 0 ? "Recommended" : "Eligible swap" }));
  const podOptions = podNames.map((name) => ({ name, state: "Selected pod" }));
  const row = (label: string, copy: string) => ({ label, copy });
  const simple = (title: string, eyebrow: string, copy: string, chips?: string[], action = "Keep recommended") => setDetail({ title, eyebrow, copy, chips, action });
  const chooseFunctional = () => setDetail({ title: "Functional support options", eyebrow: "Care", copy: "Functional support is the practical care layer — nutrition, movement, recovery, LED, acupuncture, pelvic floor, breathwork, or body support depending the pathway. Choose one support option for this month.", rows: [row("Recommended for this pathway", activation.functional), row("Pathway default", d["Functional Care"])], options: functionalOptions, selectedOption: chosenFunctional, onSelect: setChosenFunctional, action: "Choose functional support" });
  const chooseKit = () => setDetail({ title: "Build kit", eyebrow: "Kit", copy: "The kit is the at-home support for the work your month is asking you to do. Choose the recommended item or one eligible swap; the pathway and kit purpose stay the same.", rows: [row("Recommended kit", activation.kit), row("What stays the same", "One kit lane supports the same pathway goal; pack-only upgrades and premium boosts remain gated.")], options: kitOptions, selectedOption: chosenKit, onSelect: setChosenKit, action: "Build kit" });
  const choosePass = () => setDetail({ title: "Choose pass", eyebrow: "Pass + Unlocks", copy: "The pass is one bookable monthly experience such as breathwork, movement, recovery, LED, workshop, or partner demo. Choose one pass from this pathway's available options.", rows: [row("Recommended pass set", activation.passes)], options: passOptions, selectedOption: chosenPass, onSelect: setChosenPass, action: "Choose pass" });
  const podAgenda = (title = "Pod agenda") => setDetail({ title, eyebrow: "Coach", copy: "Each pod includes a focused agenda and output. Use the agenda to understand what the pod covers before swapping one pod.", rows: podNames.map((pod) => row(pod, podAgendas[pod] || activation.podCovers)), options: podOptions, selectedOption: chosenPod, onSelect: setChosenPod, action: "Swap one pod" });
  const mbcDetail = () => setDetail({ title: "Milestone Bonus Credits", eyebrow: "Pass + Unlocks", copy: "MBC are Stretch-funded credits earned by completing plan actions.", rows: [row("Earn with", "kit built, pod joined, pass booked, labs completed, coaching done, 7-day streak"), row("Use for", "selected packs, kit upgrades, device buy-downs, future pathway add-ons, special partner experiences")], action: "View Wallet" });
  const futureDetail = () => setDetail({ title: "Future unlocks", eyebrow: "Pass + Unlocks", copy: "These are previews, not active day-one benefits. They can become locked, preview, eligible, or active depending on progress, eligibility, or clinician review.", rows: [row("Packs", packsCatalog[key].join(", ")), row("Devices", [pathway.futureDevice, ...futureCatalog.filter((item) => /Ring|LED Mask|CGM|red-light/i.test(item)).slice(0, 3)].join(", ")), row("Riders", [pathway.futureRider, ...splitBlueprintList(activation.future).filter((item) => /Rider|Shield|Rx|DermaShield|NeuroSleep/i.test(item))].join(", ")), row("Adjacent pathways", pathways[pathway.adjacent].title), row("Activation future", activation.future)], action: "See future unlocks" });

  const stackCards = [
    { label: "Your priority", copy: pathway.title, open: () => setDetail({ title: "Your priority", eyebrow: "Stack", copy: pathway.reason, rows: [row("Pathway", pathway.title), row("Monthly promise", pathway.monthlyPromise), row("Why Stretch chose it", pathway.reason)], action: "Keep priority" }) },
    { label: "Care route", copy: activation.specialist, open: () => setDetail({ title: "Care route", eyebrow: "Stack", copy: "This is the expert route that anchors the month and keeps the plan safe.", rows: [row("Specialist", activation.specialist), row("Functional support", activation.functional), row("Clinical / LED / review", activation.clinical), row("Labs", activation.labs)], action: "Review care" }) },
    { label: "Functional support", copy: chosenFunctional || activation.functional, open: chooseFunctional },
    { label: "Coach + pods", copy: activation.pods, open: () => setDetail({ title: "Coach + pods", eyebrow: "Stack", copy: "Pods create the guided rhythm; coaching turns that rhythm into weekly action.", rows: [row("Coaching touch 1", activation.coach1), row("Coaching touch 2", activation.coach2), row("Pods", activation.pods), row("Pod covers", activation.podCovers)], action: "View pod agenda" }) },
    { label: "Kit", copy: chosenKit || activation.kit, open: () => setDetail({ title: "Kit", eyebrow: "Stack", copy: "The kit is the at-home support for the work your month is asking you to do.", rows: [row("Recommended kit", activation.kit), row("Catalog options", kitCatalog[key].join(", "))], options: kitOptions, selectedOption: chosenKit, onSelect: setChosenKit, action: "Build kit" }) },
  ...
  return <section className="relative space-y-5 px-5 py-7"><div className="space-y-2"><p className="text-sm font-bold text-accent">Activation Review</p><h1 className="font-display text-4xl leading-tight">Your Stretch month is ready.</h1><p className="text-base leading-7 text-muted-foreground">Review what unlocked, choose what needs your input, and start your first week.</p></div><div className="sticky top-0 z-10 -mx-5 bg-shell/95 px-5 py-2 backdrop-blur-xl"><div className="flex gap-2 overflow-x-auto pb-1">{sectionTabs.map((tab) => <button key={tab} onClick={() => setActive(tab)} className={cn("shrink-0 rounded-full px-4 py-2 text-xs font-bold transition-smooth focus-visible:outline-none focus-visible:ring-2 focus-visible:ring-ring", active === tab ? "bg-primary text-primary-foreground shadow-card" : "bg-card text-accent shadow-card")}>{tab}</button>)}</div></div>{active === "Stack" && <ActivationPanel eyebrow="Section 1" title="Your Month Stack" copy="A high-level view of the complete first month: priority, care, functional support, coach, pods, kit, pass, progress, and future unlocks."><div className="grid gap-3">{stackCards.map((card) => <ActivationInfoCard key={card.label} label={card.label} copy={card.copy} onOpen={card.open} />)}</div><Button variant="hero" size="xl" className="mt-4 w-full" onClick={() => setActive("Care")}>Review care</Button></ActivationPanel>}{active === "Care" && <ActivationPanel eyebrow="Section 2" title="Care" copy="Specialist routing, functional support, clinical review, LED loops, and diagnostics are shown for this exact pathway."><div className="grid gap-3"><ActivationInfoCard label="Specialist route" copy={activation.specialist} onOpen={() => setDetail({ title: "Specialist route", eyebrow: "Care", copy: "This is the expert route that anchors the month and keeps the plan safe.", rows: [row("Route", activation.specialist), row("Pathway default", d.Specialist), row("Why", pathway.reason)], action: "Keep specialist route" })} /><ActivationInfoCard label="Functional support options" copy={chosenFunctional || activation.functional} onOpen={chooseFunctional} /><ActivationInfoCard label="Clinical / LED / review route" copy={activation.clinical} onOpen={() => setDetail({ title: "Clinical / LED / review route", eyebrow: "Care", copy: "This is the check, review, LED, or clinical loop that determines whether the month stays simple or needs escalation.", rows: [row("Unlocked route", activation.clinical), row("Pathway default", d["Clinical / LED / Review"])], action: "Keep review route" })} /><ActivationInfoCard label="Diagnostics / labs" copy={activation.labs} onOpen={() => setDetail({ title: "Diagnostics / labs", eyebrow: "Care", copy: "Labs are not a casual marketplace item. They appear when useful to clarify fatigue, hormones, inflammation, metabolism, recovery, or symptom patterns.", rows: diagnosticsCatalog[key].map((item, index) => row(index === 0 ? "Included / current" : item.split(":")[0], item)), action: "Keep diagnostics" })} /><ActivationInfoCard label="Choose functional support" copy="Open selectable support options" onOpen={chooseFunctional} /></div><ActivationButtons primary="Choose functional support" secondary="Keep recommended" onPrimary={chooseFunctional} onContinue={() => setActive("Coach")} onSecondary={onKeep} /></ActivationPanel>}{active === "Coach" && <ActivationPanel eyebrow="Section 3" title="Coach" copy="Selected pods, pod outputs, coaching touches, behavioral support, and agenda rows are connected to this pathway."><div className="grid gap-3"><ActivationInfoCard label="Selected pods" copy={chosenPod || activation.pods} onOpen={() => podAgenda("Selected pods")} /><ActivationInfoCard label="What each pod covers" copy={activation.podCovers} onOpen={() => setDetail({ title: "What each pod covers", eyebrow: "Coach", copy: "Pod-specific agendas define the work, expected output, and why the pod belongs in this month.", rows: podNames.map((pod) => row(pod, podAgendas[pod] || activation.podCovers)), action: "View pod agenda" })} /><ActivationInfoCard label="Coaching touch 1" copy={activation.coach1} onOpen={() => simple("Coaching touch 1", "Coach", "This touch sets up the first week and converts the pod theme into an action plan.", [activation.coach1])} /><ActivationInfoCard label="Coaching touch 2" copy={activation.coach2} onOpen={() => simple("Coaching touch 2", "Coach", "This touch reviews whether the plan is usable and adjusts it.", [activation.coach2])} /><ActivationInfoCard label="Mental / behavioral support" copy={activation.mental} onOpen={() => simple("Mental / behavioral support", "Coach", "This helps with stress, mood, pain, adherence, body image, or brain fog when those factors make the month harder to complete.", splitBlueprintList(activation.mental))} /><ActivationInfoCard label="Pod agenda" copy="Open rows for each pod with agenda and output" onOpen={() => podAgenda("Pod agenda")} /></div><ActivationButtons primary="View pod agenda" secondary="Swap one pod" onPrimary={() => podAgenda("Pod agenda")} onContinue={() => setActive("Kit")} onSecondary={onSwap} /></ActivationPanel>}{active === "Kit" && <ActivationPanel eyebrow="Section 4" title="Kit" copy="Kit options, one eligible swap, sticky perks, and pack-only upgrades are separated so the kit does not feel like a generic list."><div className="grid gap-3"><ActivationInfoCard label="Recommended kit" copy={chosenKit || activation.kit} onOpen={() => setDetail({ title: "Recommended kit", eyebrow: "Kit", copy: "The kit is the at-home support for the work your month is asking you to do.", rows: [row("Recommended kit", activation.kit), row("Catalog detail", kitCatalog[key].join(", "))], options: kitOptions, selectedOption: chosenKit, onSelect: setChosenKit, action: "Build kit" })} /><ActivationInfoCard label="What can be swapped" copy={kitCatalog[key][1] || kitCatalog[key][0]} onOpen={() => setDetail({ title: "What can be swapped", eyebrow: "Kit", copy: "One eligible kit swap is allowed. The item can change, but the pathway goal and at-home support purpose stay the same.", rows: [row("Eligible swap", kitCatalog[key][1] || kitCatalog[key][0]), row("What stays the same", activation.kit)], options: kitOptions.slice(0, 3), selectedOption: chosenKit, onSelect: setChosenKit, action: "Swap one kit item" })} /><ActivationInfoCard label="Sticky perk option" copy={stickyPerks.slice(0, 4).join(", ")} onOpen={() => simple("Sticky perk option", "Kit", "Sticky perks are small experiential benefits that make the month feel useful and rewarding.", stickyPerks, "Choose sticky perk")} /><ActivationInfoCard label="Pack-only upgrades" copy={packsCatalog[key].slice(0, 3).join(", ")} onOpen={() => setDetail({ title: "Pack-only upgrades", eyebrow: "Kit", copy: "Packs are boosts, not required for the plan to be useful.", rows: packsCatalog[key].map((pack, index) => row(index === 0 ? "Strongest pack" : "Pack preview", pack)), action: "Mark interest" })} /><ActivationInfoCard label="Build kit" copy="Open selectable kit options" onOpen={chooseKit} /></div><ActivationButtons primary="Build kit" secondary="Swap one kit item" onPrimary={chooseKit} onContinue={() => setActive("Pass + Unlocks")} onSecondary={onSwap} /></ActivationPanel>}{active === "Pass + Unlocks" && <ActivationPanel eyebrow="Section 5" title="Pass + Unlocks" copy="Experience pass selection, Milestone Bonus Credits, earn actions, credit uses, and pathway-specific future previews."><div className="grid gap-3"><ActivationInfoCard label="Experience pass" copy={chosenPass || activation.passes} onOpen={() => setDetail({ title: "Experience pass", eyebrow: "Pass + Unlocks", copy: "The pass is one bookable monthly experience such as breathwork, movement, recovery, LED, workshop, or partner demo.", options: passOptions, selectedOption: chosenPass, onSelect: setChosenPass, action: "Choose pass" })} /><ActivationInfoCard label="Choose pass" copy="Open selectable pass options" onOpen={choosePass} /><ActivationInfoCard label="Milestone Bonus Credits" copy="Stretch-funded credits earned by completing plan actions" onOpen={mbcDetail} /><ActivationInfoCard label="Earn with" copy="kit built, pod joined, pass booked, labs completed, coaching done, 7-day streak" onOpen={() => simple("Earn with", "Pass + Unlocks", "These are the actions that earn Milestone Bonus Credits when completed.", ["kit built", "pod joined", "pass booked", "labs completed", "coaching done", "7-day streak"], "View Wallet")} /><ActivationInfoCard label="Use for" copy="selected packs, kit upgrades, device buy-downs, future pathway add-ons, special partner experiences" onOpen={() => simple("Use for", "Pass + Unlocks", "Credits can support selected next-step benefits once the plan is ready for them.", ["selected packs", "kit upgrades", "device buy-downs", "future pathway add-ons", "special partner experiences"], "View Wallet")} /><ActivationInfoCard label="Future unlocks" copy={activation.future} onOpen={futureDetail} /></div><div className="mt-4 grid gap-3"><Button variant="soft" size="lg" onClick={choosePass}>Choose pass</Button><Button variant="soft" size="lg" onClick={mbcDetail}>View Wallet</Button><Button variant="soft" size="lg" onClick={futureDetail}>See future unlocks</Button><Button variant="hero" size="xl" onClick={onBuild}>Go to full plan</Button></div></ActivationPanel>}{detail && <ActivationDetailDrawer detail={detail} onClose={() => setDetail(null)} />}</section>;
}
