...
function BlueprintDrawer({ title, block, pathway, selectedOption, onSelect, onClose, onSwap }: { title: BlueprintTitle; block: MonthBlock; pathway: Pathway; selectedOption?: string; onSelect: (name: string) => void; onClose: () => void; onSwap: (target?: string) => void }) {
  const catalog = catalogForBlock(block, pathway);
  const lockedOptions = catalog.options.filter((option) => /locked|future|rider|pack|clinician-gated/i.test(option.state || ""));
  const futureCopy = lockedOptions.length ? lockedOptions.map((option) => `${option.name}${option.state ? ` — ${option.state}` : ""}`).join(" • ") : "Bigger upgrades appear later through packs, devices, or riders.";
  const renderOption = (option: CatalogOption) => {
    const selected = selectedOption === option.name;
    return <button key={option.name} onClick={() => onSelect(option.name)} className={cn("rounded-2xl p-4 text-left shadow-card transition-smooth hover:-translate-y-0.5 focus-visible:outline-none focus-visible:ring-2 focus-visible:ring-ring", selected ? "bg-primary text-primary-foreground" : "bg-secondary")}><span className="flex items-start justify-between gap-3"><span><span className="text-sm font-semibold">{option.name}</span>{option.state && <span className={cn("mt-2 inline-flex rounded-full px-3 py-1 text-[11px] font-bold", selected ? "bg-primary-foreground/15 text-primary-foreground" : "bg-card text-accent")}>{option.state}</span>}</span>{selected && <Check className="mt-0.5 size-4 shrink-0" />}</span></button>;
  };
  return <div className="absolute inset-0 z-50 flex items-end bg-primary/25 p-3 backdrop-blur-sm" onClick={onClose}><div className="max-h-[84vh] w-full overflow-y-auto rounded-[2rem] bg-card p-6 shadow-float animate-slide-up" onClick={(event) => event.stopPropagation()}><div className="mb-5 flex items-start justify-between gap-3"><div className="flex items-start gap-3"><div className="rounded-full bg-secondary p-3 text-accent">{iconForBlock(title)}</div><div><p className="text-sm font-bold text-accent">Blueprint detail</p><h2 className="font-display text-3xl leading-tight">{title}</h2>{selectedOption && <p className="mt-1 text-xs font-bold text-accent">Selected: {selectedOption}</p>}</div></div><button onClick={onClose} className="rounded-full bg-secondary px-3 py-2 text-sm font-semibold text-accent focus-visible:outline-none focus-visible:ring-2 focus-visible:ring-ring">Close</button></div><div className="grid gap-3"><InfoBlock label="What this means" copy={block.plain} /><InfoBlock label="Why this matters" copy={block.discovery} /><InfoBlock label="What Stretch recommended" copy={`${block.selection}. Includes: ${block.includes}`} /><div className="rounded-2xl bg-secondary p-4"><p className="text-xs font-semibold uppercase tracking-wide text-accent">Available components</p><div className="mt-3 grid gap-2">{catalog.options.map(renderOption)}</div></div><InfoBlock label="What you can choose or swap" copy={block.swappable ? "You can swap one part of this block this month." : "This is guided or locked for safety / eligibility."} /><InfoBlock label="What is locked or future" copy={futureCopy} /></div><div className="mt-5 grid gap-3"><Button variant="hero" size="xl" onClick={onClose}>Keep this piece</Button>{block.swappable && <Button variant="soft" size="xl" onClick={() => onSwap(swapTargetForBlueprint(title))}>Swap this block</Button>}<Button variant="soft" size="xl" onClick={onClose}>Ask coach</Button></div></div></div>;
}
...
function UnlocksScreen({ pathway, onBuild, onKeep, onSwap, onCoach }: { pathway: Pathway; answers: string[]; onBuild: () => void; onKeep: () => void; onSwap: () => void; onCoach: () => void }) {
  ...
  const chooseKit = () => setDetail({ title: "Build kit", eyebrow: "Kit", copy: "The kit is the at-home support for the work your month is asking you to do.", rows: [row("Recommended kit", activation.kit), row("Catalog detail", kitCatalog[key].join(", "))], action: "Build kit", actionType: "build-kit" });
  const choosePass = () => setDetail({ title: "Choose pass", eyebrow: "Pass + Unlocks", copy: "The pass is one bookable monthly experience such as breathwork, movement, recovery, LED, workshop, or partner demo.", rows: [row("Recommended pass set", activation.passes)], action: "Choose pass", actionType: "choose-pass" });
  const podAgenda = (title = "Pod agenda") => setDetail({ title, eyebrow: "Coach", copy: "Each pod includes a focused agenda and output. Use the agenda to understand what the pod covers before swapping one pod.", rows: podNames.map((pod) => row(pod, podAgendas[pod] || activation.podCovers)), action: "View pod agenda", actionType: "view-pod-agenda" });
  ...
  const stackCards = [
    { label: "Your priority", copy: pathway.title, open: () => setDetail({ title: "Your priority", eyebrow: "Stack", copy: pathway.reason, rows: [row("Pathway", pathway.title), row("Monthly promise", pathway.monthlyPromise), row("Why Stretch chose it", pathway.reason)], action: "Ask coach", actionType: "ask-coach" }) },
  ...
  return <section className="relative space-y-5 px-5 py-7"><div className="space-y-2"><p className="text-sm font-bold text-accent">Activation Review</p><h1 className="font-display text-4xl leading-tight">Your Stretch month is ready.</h1><p className="text-base leading-7 text-muted-foreground">Review what unlocked, choose what needs your input, and start your first week.</p></div><div className="sticky top-0 z-10 -mx-5 bg-shell/95 px-5 py-2 backdrop-blur-xl"><div className="flex gap-2 overflow-x-auto pb-1">{sectionTabs.map((tab) => <button key={tab} onClick={() => setActive(tab)} className={cn("shrink-0 rounded-full px-4 py-2 text-xs font-bold transition-smooth focus-visible:outline-none focus-visible:ring-2 focus-visible:ring-ring", active === tab ? "bg-primary text-primary-foreground shadow-card" : "bg-card text-accent shadow-card")}>{tab}</button>)}</div></div>{active === "Stack" && <ActivationPanel eyebrow="Section 1" title="Your Month Stack" copy="A high-level view of the complete first month: priority, care, functional support, coach, pods, kit, pass, progress, and future unlocks."><div className="grid gap-3">{stackCards.map((card) => <ActivationInfoCard key={card.label} label={card.label} copy={card.copy} onOpen={card.open} />)}</div><Button variant="hero" size="xl" className="mt-4 w-full" onClick={() => setActive("Care")}>Review care</Button></ActivationPanel>}{active === "Care" && <ActivationPanel eyebrow="Section 2" title="Care" copy="Specialist routing, functional support, clinical review, LED loops, and diagnostics are shown for this exact pathway."><div className="grid gap-3"><ActivationInfoCard label="Specialist route" copy={activation.specialist} onOpen={() => setDetail({ title: "Specialist route", eyebrow: "Care", copy: "This is the expert route that anchors the month and keeps the plan safe.", rows: [row("Route", activation.specialist), row("Pathway default", d.Specialist), row("Why", pathway.reason)], action: "Ask coach", actionType: "ask-coach" })} /><ActivationInfoCard label="Functional support options" copy={chosenFunctional || activation.functional} onOpen={chooseFunctional} /><ActivationInfoCard label="Clinical / LED / review route" copy={activation.clinical} onOpen={() => setDetail({ title: "Clinical / LED / review route", eyebrow: "Care", copy: "This is the check, review, LED, or clinical loop that determines whether the month stays simple or needs escalation.", rows: [row("Unlocked route", activation.clinical), row("Pathway default", d["Clinical / LED / Review"])], action: "Ask coach", actionType: "ask-coach" })} /><ActivationInfoCard label="Diagnostics / labs" copy={activation.labs} onOpen={() => setDetail({ title: "Diagnostics / labs", eyebrow: "Care", copy: "Labs are not a casual marketplace item. They appear when useful to clarify fatigue, hormones, inflammation, metabolism, recovery, or symptom patterns.", rows: diagnosticsCatalog[key].map((item, index) => row(index === 0 ? "Included / current" : item.split(":")[0], item)), action: "Ask coach", actionType: "ask-coach" })} /><ActivationInfoCard label="Choose functional support" copy="Open selectable support options" onOpen={() => setActivePicker("functional")} /></div><ActivationButtons primary="Choose functional support" secondary="Keep recommended" onPrimary={() => setActivePicker("functional")} onContinue={() => setActive("Coach")} onSecondary={onKeep} /></ActivationPanel>}{active === "Coach" && <ActivationPanel eyebrow="Section 3" title="Coach" copy="Selected pods, pod outputs, coaching touches, behavioral support, and agenda rows are connected to this pathway."><div className="grid gap-3"><ActivationInfoCard label="Selected pods" copy={chosenPod || activation.pods} onOpen={() => podAgenda("Selected pods")} /><ActivationInfoCard label="What each pod covers" copy={activation.podCovers} onOpen={() => setDetail({ title: "What each pod covers", eyebrow: "Coach", copy: "Pod-specific agendas define the work, expected output, and why the pod belongs in this month.", rows: podNames.map((pod) => row(pod, podAgendas[pod] || activation.podCovers)), action: "View pod agenda", actionType: "view-pod-agenda" })} /><ActivationInfoCard label="Coaching touch 1" copy={activation.coach1} onOpen={() => simple("Coaching touch 1", "Coach", "This touch sets up the first week and converts the pod theme into an action plan.", [activation.coach1], "Ask coach", "ask-coach")} /><ActivationInfoCard label="Coaching touch 2" copy={activation.coach2} onOpen={() => simple("Coaching touch 2", "Coach", "This touch reviews whether the plan is usable and adjusts it.", [activation.coach2], "Ask coach", "ask-coach")} /><ActivationInfoCard label="Mental / behavioral support" copy={activation.mental} onOpen={() => simple("Mental / behavioral support", "Coach", "This helps with stress, mood, pain, adherence, body image, or brain fog when those factors make the month harder to complete.", splitBlueprintList(activation.mental), "Ask coach", "ask-coach")} /><ActivationInfoCard label="Pod agenda" copy="Open rows for each pod with agenda and output" onOpen={() => setActivePicker("pods")} /></div><ActivationButtons primary="View pod agenda" secondary="Swap one pod" onPrimary={() => setActivePicker("pods")} onContinue={() => setActive("Kit")} onSecondary={() => setActivePicker("pods")} /></ActivationPanel>}{active === "Kit" && <ActivationPanel eyebrow="Section 4" title="Kit" copy="Kit options, one eligible swap, sticky perks, and pack-only upgrades are separated so the kit does not feel like a generic list."><div className="grid gap-3"><ActivationInfoCard label="Recommended kit" copy={chosenKit || activation.kit} onOpen={chooseKit} /><ActivationInfoCard label="What can be swapped" copy={kitCatalog[key][1] || kitCatalog[key][0]} onOpen={() => setDetail({ title: "What can be swapped", eyebrow: "Kit", copy: "One eligible kit swap is allowed. The item can change, but the pathway goal and at-home support purpose stay the same.", rows: [row("Eligible swap", kitCatalog[key][1] || kitCatalog[key][0]), row("What stays the same", activation.kit)], action: "Build kit", actionType: "build-kit" })} /><ActivationInfoCard label="Sticky perk option" copy={stickyPerks.slice(0, 4).join(", ")} onOpen={() => simple("Sticky perk option", "Kit", "Sticky perks are small experiential benefits that make the month feel useful and rewarding.", stickyPerks, "Build kit", "build-kit")} /><ActivationInfoCard label="Pack-only upgrades" copy={packsCatalog[key].slice(0, 3).join(", ")} onOpen={() => setDetail({ title: "Pack-only upgrades", eyebrow: "Kit", copy: "Packs are boosts, not required for the plan to be useful.", rows: packsCatalog[key].map((pack, index) => row(index === 0 ? "Strongest pack" : "Pack preview", pack)), action: "See future unlocks", actionType: "see-future-unlocks" })} /><ActivationInfoCard label="Build kit" copy="Open selectable kit options" onOpen={() => setActivePicker("kit")} /></div><ActivationButtons primary="Build kit" secondary="Swap one kit item" onPrimary={() => setActivePicker("kit")} onContinue={() => setActive("Pass + Unlocks")} onSecondary={() => setActivePicker("kit")} /></ActivationPanel>}{active === "Pass + Unlocks" && <ActivationPanel eyebrow="Section 5" title="Pass + Unlocks" copy="Experience pass selection, Milestone Bonus Credits, earn actions, credit uses, and pathway-specific future previews."><div className="grid gap-3"><ActivationInfoCard label="Experience pass" copy={chosenPass || activation.passes} onOpen={choosePass} /><ActivationInfoCard label="Choose pass" copy="Open selectable pass options" onOpen={() => setActivePicker("pass")} /><ActivationInfoCard label="Milestone Bonus Credits" copy="Stretch-funded credits earned by completing plan actions" onOpen={mbcDetail} /><ActivationInfoCard label="Earn with" copy="kit built, pod joined, pass booked, labs completed, coaching done, 7-day streak" onOpen={() => simple("Earn with", "Pass + Unlocks", "These are the actions that earn Milestone Bonus Credits when completed.", ["kit built", "pod joined", "pass booked", "labs completed", "coaching done", "7-day streak"], "Learn about MBC", "learn-mbc")} /><ActivationInfoCard label="Use for" copy="selected packs, kit upgrades, device buy-downs, future pathway add-ons, special partner experiences" onOpen={() => simple("Use for", "Pass + Unlocks", "Credits can support selected next-step benefits once the plan is ready for them.", ["selected packs", "kit upgrades", "device buy-downs", "future pathway add-ons", "special partner experiences"], "View Wallet", "view-wallet")} /><ActivationInfoCard label="Future unlocks" copy={activation.future} onOpen={futureDetail} /></div><div className="mt-4 grid gap-3"><Button variant="soft" size="lg" onClick={() => setActivePicker("pass")}>Choose pass</Button><Button variant="soft" size="lg" onClick={() => setActivePicker("mbc")}>View Wallet</Button><Button variant="soft" size="lg" onClick={() => setActivePicker("future")}>See future unlocks</Button><Button variant="hero" size="xl" onClick={onBuild}>Go to full plan</Button></div></ActivationPanel>}{detail && <ActivationDetailDrawer detail={detail} onClose={() => setDetail(null)} onAction={handleAction} />}{activePicker && <ActivationPickerDrawer type={activePicker} pathway={pathway} activation={activation} selected={pickerSelected} onSelect={pickerSelect} onClose={() => setActivePicker(null)} onSwap={onSwap} onCoach={onCoach} />}</section>;
}
